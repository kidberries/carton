...
class CTN_Widget_Layered_Nav extends WP_Widget {
	 ...
	 * @return void
	 */
	function CTN_Widget_Layered_Nav() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass 		= 'carton widget_layered_nav';
		$this->woo_widget_description	= __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'carton' );
		$this->woo_widget_idbase 		= 'carton_layered_nav';
		$this->woo_widget_name 			= __( 'CartoN Layered Nav', 'carton' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->woo_widget_cssclass, 'description' => $this->woo_widget_description );

		/* Create the widget. */
		$this->WP_Widget( 'carton_layered_nav', $this->woo_widget_name, $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$taxonomy 		= $carton->attribute_taxonomy_name($instance['attribute']);
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'carton' ), $carton->attribute_label( $taxonomy ) ) .'</option>';
					...
					foreach ( $terms as $term ) {
						...
						$transient_name = 'ctn_ln_count_' . md5( sanitize_key( $taxonomy ) . sanitize_key( $term->term_id ) );
						...
						if ( $query_type == 'and' ) {
							...
							$count = sizeof( array_intersect( $_products_in_term, $carton->query->filtered_product_ids ) );
						...
						// If this is an OR query, show all options so search can be expanded
							...
							$count = sizeof( array_intersect( $_products_in_term, $carton->query->unfiltered_product_ids ) );
				...
				foreach ( $terms as $term ) {
					...
					echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'carton_layered_nav_link', $link ) ) . '">' : '<span>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) )
			$new_instance['title'] = $carton->attribute_label( $new_instance['attribute'] );
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'attribute' ); ?>"><?php _e( 'Attribute:', 'carton' ) ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', 'carton' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', 'carton' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', 'carton' ); ?></option>
		</select></p>

		<p><label for="<?php echo $this->get_field_id( 'query_type' ); ?>"><?php _e( 'Query Type:', 'carton' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'query_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'query_type' ) ); ?>">
			<option value="and" <?php selected( $instance['query_type'], 'and' ); ?>><?php _e( 'AND', 'carton' ); ?></option>
			<option value="or" <?php selected( $instance['query_type'], 'or' ); ?>><?php _e( 'OR', 'carton' ); ?></option>
		</select></p>
		<?php
	}
}