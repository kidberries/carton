...
class CTN_Tax {
	...
	public function find_rates( $args = array(), $deprecated_state = null, $deprecated_postcode = null, $deprecated_class = null ) {
		...
		// Handle postcodes
		$valid_postcodes 	= array( '*', strtoupper( carton_clean( $postcode ) ) );

		// Work out possible valid wildcard postcodes
		$postcode_length	= strlen( $postcode );
		$wildcard_postcode	= strtoupper( carton_clean( $postcode ) );
		...
		$found_rates = $wpdb->get_results( $wpdb->prepare( "
			SELECT tax_rates.* FROM
				{$wpdb->prefix}carton_tax_rates as tax_rates
			LEFT OUTER JOIN
				{$wpdb->prefix}carton_tax_rate_locations as locations ON tax_rates.tax_rate_id = locations.tax_rate_id
                        INNER JOIN (
                                SELECT tax_rate_priority FROM {$wpdb->prefix}carton_tax_rates 
                                        GROUP BY tax_rate_priority 
                                    ) AS tax_sec ON tax_sec.tax_rate_priority = tax_rates.tax_rate_priority            
			WHERE
		...
		return apply_filters( 'carton_matched_tax_rates', $matched_tax_rates, $country, $state, $postcode, $city, $tax_class );
	}
	...
	public function is_compound( $key ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_compound FROM {$wpdb->prefix}carton_tax_rates WHERE tax_rate_id = %s", $key ) ) ? true : false;
	}
	...
	public function get_rate_label( $key ) {
		global $wpdb;
		return apply_filters( 'carton_rate_label', $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_name FROM {$wpdb->prefix}carton_tax_rates WHERE tax_rate_id = %s", $key ) ), $key, $this );
	}
	...
	public function get_rate_code( $key ) {
		...
		$rate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}carton_tax_rates WHERE tax_rate_id = %s", $key ) );
		...
		return apply_filters( 'carton_rate_code', strtoupper( implode( '-', array_filter( $code ) ) ), $key, $this );
	}
	 ...
	 * }
	 * add_filter( 'carton_tax_round', 'euro_5cent_rounding' );
	 *
	 */
	public function round( $in ) {
		return apply_filters( 'carton_tax_round', round( $in, $this->dp ), $in );
	}
