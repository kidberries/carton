jQuery(document).ready(function($) {
	...
	// Shipping calculator
	$(document).on( 'change', 'input[name="shipping_method"], .shipping_method_variant, .shipping_method_sub_variant, select.russianpost_places.ajax-chzn-select', function() {

        var data = {
            action:			'get_dynamic_shipping',
            security:		carton_params.update_shipping_method_nonce,
			product_id:		$("form.cart").data("product_id"),
...
*/
	        $('#shipping_method').block({message: null, overlayCSS: {background: '#fff url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});
		...
		$.post( carton_params.ajax_url, data, function(response) {
			...
			$('#shipping_method').replaceWith( response );
