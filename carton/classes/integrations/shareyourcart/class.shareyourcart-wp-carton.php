...
if ( ! class_exists( 'ShareYourCartCartoN', false ) ) {

	/**
	 * ShareYourCartCartoN class.
	 *
	 * @extends ShareYourCartWordpressPlugin
	 */
	class ShareYourCartCartoN extends ShareYourCartWordpressPlugin{
	    ...
	    public function isCartActive() {
		    return in_array( 'carton/carton.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) );
	    }
		...
		public function pluginsLoadedHook() {
			...
			add_action( 'carton_before_single_product', array( $this, 'showProductButton' ) );
			add_action( 'carton_cart_contents', array( $this, 'showCartButton' ) );
			add_action( 'carton_checkout_before_customer_details', array( $this, 'showCheckoutButton' ) );
		}
	    ...
	    /**
	     * This function is to be used by CartoN from version 2.0
	     *
	     * @access public
	     * @return void
	     */
	    public function buttonCallback20(){
			...
			$params = array(
				...
				'success_url'  => get_bloginfo('wpurl').'/?page_id='.get_option('carton_cart_page_id'),
				'cancel_url'   => get_bloginfo('wpurl').'/?page_id='.get_option('carton_cart_page_id'),
			);
			...
			if ( ! isset( $_REQUEST['p'] ) ) {
				...
				foreach( $carton->session->cart as $cart_details ) {
		...
		private function _getProductDetails20( $product_id ) {
			...
			//CartoN actually echoes the image
			ob_start();
			echo $product->get_image(); // older CartoN versions might allready echo, but newer versions don't, so force it anyway
			$image = ob_get_clean();
		...
		/**
		 * _loadCartoN function.
		 *
		 * @access private
		 * @return void
		 */
		private function _loadCartoN(){
			// Sometimes the CartoN Class is not loaded...
			if ( ! class_exists( 'Carton', false ) ) {
				require_once( ABSPATH . 'wp-content/plugins/carton/carton.php' );
			}
	...
	add_action( ShareYourCartWordpressPlugin::getPluginFile(), array( 'ShareYourCartCartoN','uninstallHook' ) );
}
