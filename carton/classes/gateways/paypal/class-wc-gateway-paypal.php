 ...
 *
 * @class 		CTN_Paypal
 * @extends		CTN_Gateway_Paypal
 * @version		2.0.0
 * @package		CartoN/Classes/Payment
 * @author 		CartonThemes
 */
class CTN_Gateway_Paypal extends CTN_Payment_Gateway {
	...
	public function __construct() {
        ...
        $this->id           = 'paypal';
        $this->icon         = apply_filters( 'carton_paypal_icon', $carton->plugin_url() . '/assets/images/icons/paypal.png' );
        $this->has_fields   = false;
        $this->liveurl      = 'https://www.paypal.com/cgi-bin/webscr';
		$this->testurl      = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
        $this->method_title = __( 'PayPal', 'carton' );
        $this->notify_url   = str_replace( 'https:', 'http:', add_query_arg( 'wc-api', 'CTN_Gateway_Paypal', home_url( '/' ) ) );
		...
		// Logs
		if ( 'yes' == $this->debug )
			$this->log = $carton->logger();

		// Actions
		add_action( 'valid-paypal-standard-ipn-request', array( $this, 'successful_request' ) );
		add_action( 'carton_receipt_paypal', array( $this, 'receipt_page' ) );
		add_action( 'carton_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );

		// Payment listener/API hook
		add_action( 'carton_api_ctn_gateway_paypal', array( $this, 'check_ipn_response' ) );
    ...
    function is_valid_for_use() {
        if ( ! in_array( get_carton_currency(), apply_filters( 'carton_paypal_supported_currencies', array( 'AUD', 'BRL', 'CAD', 'MXN', 'NZD', 'HKD', 'SGD', 'USD', 'EUR', 'JPY', 'TRY', 'NOK', 'CZK', 'DKK', 'HUF', 'ILS', 'MYR', 'PHP', 'PLN', 'SEK', 'CHF', 'TWD', 'THB', 'GBP', 'RMB' ) ) ) ) return false;
	...
	public function admin_options() {
		...
		?>
		<h3><?php _e( 'PayPal standard', 'carton' ); ?></h3>
		<p><?php _e( 'PayPal standard works by sending the user to PayPal to enter their payment information.', 'carton' ); ?></p>
		...
		<?php else : ?>
            <div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'carton' ); ?></strong>: <?php _e( 'PayPal does not support your store currency.', 'carton' ); ?></p></div>
		<?php
			endif;
	}
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			'enabled' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Enable PayPal standard', 'carton' ),
							'default' => 'yes'
						),
			'title' => array(
			...
			'description' => array(
							...
							'default' => __( 'Pay via PayPal; you can pay with your credit card if you don\'t have a PayPal account', 'carton' )
						),
			'email' => array(
							'title' => __( 'PayPal Email', 'carton' ),
							'type' 			=> 'email',
							'description' => __( 'Please enter your PayPal email address; this is needed in order to take payment.', 'carton' ),
							'default' => '',
							'desc_tip'      => true,
							'placeholder'	=> 'you@youremail.com'
						),
			'receiver_email' => array(
							'title' => __( 'Receiver Email', 'carton' ),
							'type' 			=> 'email',
							'description' => __( 'If this differs from the email entered above, input your main receiver email for your PayPal account. This is used to validate IPN requests.', 'carton' ),
							'default' => '',
							'desc_tip'      => true,
							'placeholder'	=> 'you@youremail.com'
						),
			'invoice_prefix' => array(
							'title' => __( 'Invoice Prefix', 'carton' ),
							'type' => 'text',
							'description' => __( 'Please enter a prefix for your invoice numbers. If you use your PayPal account for multiple stores ensure this prefix is unique as PayPal will not allow orders with the same invoice number.', 'carton' ),
							'default' => 'WC-',
							'desc_tip'      => true,
						),
			'form_submission_method' => array(
							'title' => __( 'Submission method', 'carton' ),
							'type' => 'checkbox',
							'label' => __( 'Use form submission method.', 'carton' ),
							'description' => __( 'Enable this to post order data to PayPal via a form instead of using a redirect/querystring.', 'carton' ),
							'default' => 'no'
						),
			'page_style' => array(
							'title' => __( 'Page Style', 'carton' ),
							'type' => 'text',
							'description' => __( 'Optionally enter the name of the page style you wish to use. These are defined within your PayPal account.', 'carton' ),
							'default' => '',
							'desc_tip'      => true,
							'placeholder'	=> __( 'Optional', 'carton' )
						),
			'shipping' => array(
							'title' => __( 'Shipping options', 'carton' ),
							'type' => 'title',
							'description' => '',
						),
			'send_shipping' => array(
							'title' => __( 'Shipping details', 'carton' ),
							'type' => 'checkbox',
							'label' => __( 'Send shipping details to PayPal instead of billing.', 'carton' ),
							'description' => '',
							'description' => __( 'PayPal allows us to send 1 address. If you are using PayPal for shipping labels you may prefer to send the shipping address rather than billing.', 'carton' ),
							'default' => 'no'
						),
			'address_override' => array(
							'title' => __( 'Address override', 'carton' ),
							'type' => 'checkbox',
							'label' => __( 'Enable "address_override" to prevent address information from being changed.', 'carton' ),
							'description' => __( 'PayPal verifies addresses therefore this setting can cause errors (we recommend keeping it disabled).', 'carton' ),
							'default' => 'no'
						),
			'testing' => array(
							'title' => __( 'Gateway Testing', 'carton' ),
							'type' => 'title',
							'description' => '',
						),
			'testmode' => array(
							'title' => __( 'PayPal sandbox', 'carton' ),
							'type' => 'checkbox',
							'label' => __( 'Enable PayPal sandbox', 'carton' ),
							'default' => 'yes',
							'description' => sprintf( __( 'PayPal sandbox can be used to test payments. Sign up for a developer account <a href="%s">here</a>.', 'carton' ), 'https://developer.paypal.com/' ),
						),
			'debug' => array(
							'title' => __( 'Debug Log', 'carton' ),
							'type' => 'checkbox',
							'label' => __( 'Enable logging', 'carton' ),
							'default' => 'no',
							'description' => __( 'Log PayPal events, such as IPN requests, inside <code>carton/logs/paypal.txt</code>' ),
						)
			);
	...
	function get_paypal_args( $order ) {
		...
		$paypal_args = array_merge(
			array(
				...
				'business' 				=> $this->email,
				'no_note' 				=> 1,
				'currency_code' 		=> get_carton_currency(),
				'charset' 				=> 'UTF-8',
				'rm' 					=> is_ssl() ? 2 : 1,
				'upload' 				=> 1,
		...
		if ( get_option( 'carton_prices_include_tax' ) == 'yes' || $order->get_order_discount() > 0 ) {
			...
			$paypal_args['item_name_1'] 	= sprintf( __( 'Order %s' , 'carton'), $order->get_order_number() ) . " - " . implode( ', ', $item_names );
			...
			if ( ( $order->get_shipping() + $order->get_shipping_tax() ) > 0 ) {
				$paypal_args['item_name_2'] = __( 'Shipping via', 'carton' ) . ' ' . ucwords( $order->shipping_method_title );
			...
			if ( $order->get_shipping() > 0 ) {
				$item_loop++;
				$paypal_args[ 'item_name_' . $item_loop ] 	= __( 'Shipping via', 'carton' ) . ' ' . ucwords( $order->shipping_method_title );
				$paypal_args[ 'quantity_' . $item_loop ] 	= '1';
		...
		$paypal_args = apply_filters( 'carton_paypal_args', $paypal_args );
    ...
    function generate_paypal_form( $order_id ) {
		...
		$carton->add_inline_js( '
			jQuery("body").block({
					message: "<img src=\"' . esc_url( apply_filters( 'carton_ajax_loader_url', $carton->plugin_url() . '/assets/images/ajax-loader.gif' ) ) . '\" alt=\"Redirecting&hellip;\" style=\"float:left; margin-right: 10px;\" />'.__( 'Thank you for your order. We are now redirecting you to PayPal to make payment.', 'carton' ).'",
					overlayCSS:
					{
						background: "#fff",
						opacity: 0.6
		...
		return '<form action="'.esc_url( $paypal_adr ).'" method="post" id="paypal_payment_form" target="_top">
				...
				<input type="submit" class="button-alt" id="submit_paypal_payment_form" value="'.__( 'Pay via PayPal', 'carton' ).'" /> <a class="button cancel" href="'.esc_url( $order->get_cancel_order_url() ).'">'.__( 'Cancel order &amp; restore cart', 'carton' ).'</a>
			</form>';
	...
	function process_payment( $order_id ) {
		...
		if ( ! $this->form_submission_method ) {
			...
			return array(
				'result' 	=> 'success',
				'redirect'	=> add_query_arg('order', $order->id, add_query_arg('key', $order->order_key, get_permalink(carton_get_page_id('pay' ))))
			);
	...
	function receipt_page( $order ) {
		...
		echo '<p>'.__( 'Thank you for your order, please click the button below to pay with PayPal.', 'carton' ).'</p>';
	        ...
	        switch ( $posted['payment_status'] ) {
	            case 'completed' :
				    ...
				    if ( $order->get_total() != $posted['mc_gross'] ) {
				    	...
				    	$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'carton' ), $posted['mc_gross'] ) );
					...
					if ( strcasecmp( trim( $posted['receiver_email'] ), trim( $this->receiver_email ) ) != 0 ) {
						...
						// Put this order on-hold for manual checking
				    	$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal IPN response from a different email address (%s).', 'carton' ), $posted['receiver_email'] ) );
	            	...
	            	// Payment completed
	                $order->add_order_note( __( 'IPN payment completed', 'carton' ) );
	            ...
	            case 'voided' :
	                // Order failed
	                $order->update_status( 'failed', sprintf( __( 'Payment %s via IPN.', 'carton' ), strtolower( $posted['payment_status'] ) ) );
	            break;
	            case "refunded" :
	            	...
	            	if ( $order->get_total() == ( $posted['mc_gross'] * -1 ) ) {
		            	...
		            	$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'carton' ), strtolower( $posted['payment_status'] ) ) );

		            	$mailer = $carton->mailer();

		            	$message = $mailer->wrap_message(
		            		__( 'Order refunded/reversed', 'carton' ),
		            		sprintf( __( 'Order %s has been marked as refunded - PayPal reason code: %s', 'carton' ), $order->get_order_number(), $posted['reason_code'] )
						);

						$mailer->send( get_option( 'admin_email' ), sprintf( __( 'Payment for order %s refunded/reversed', 'carton' ), $order->get_order_number() ), $message );
	...
	function get_paypal_order( $posted ) {
		...
		if ( ! isset( $order->id ) ) {
			...
			$order_id 	= carton_get_order_id_by_order_key( $order_key );
			$order 		= new CTN_Order( $order_id );
		}
...
class CTN_Paypal extends CTN_Gateway_Paypal {
	public function __construct() {
		_deprecated_function( 'CTN_Paypal', '1.4', 'CTN_Gateway_Paypal' );
		parent::__construct();
	}
}