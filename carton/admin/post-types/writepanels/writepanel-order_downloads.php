 ...
 * @return void
 */
function carton_order_downloads_meta_box() {
	global $carton, $post, $wpdb;
	...
	<div class="order_download_permissions wc-metaboxes-wrapper">
		...
		<div class="wc-metaboxes">
			...
			<?php
				$download_permissions = $wpdb->get_results( $wpdb->prepare( "
					SELECT * FROM {$wpdb->prefix}carton_downloadable_product_permissions
		...
		<div class="toolbar">
			<p class="buttons">
				<select name="grant_access_id" class="grant_access_id chosen_select_nostd" data-placeholder="<?php _e( 'Choose a downloadable product&hellip;', 'carton' ) ?>">
					<?php
						...
						if ( $products ) foreach ( $products as $product ) {
							...
							$product_name   = carton_get_formatted_product_name( $product_object );
				...
				<button type="button" class="button grant_access"><?php _e( 'Grant Access', 'carton' ); ?></button>
			</p>
			<div class="clear"></div>
		</div>
	...
	jQuery(function(){
		...
		jQuery('.order_download_permissions').on('click', 'button.grant_access', function(){
			...
			jQuery('.order_download_permissions').block({ message: null, overlayCSS: { background: '#fff url(<?php echo $carton->plugin_url(); ?>/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			var data = {
				action: 		'carton_grant_access_to_download',
				product_id: 	product,
			...
			jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function( response ) {
				...
				if ( response ) {
					...
					alert('<?php _e( 'Could not grant access - the user may already have permission for this file or billing email is not set. Ensure the billing email is set, and the order has been saved.', 'carton' ); ?>');
		...
		jQuery('.order_download_permissions').on('click', 'button.revoke_access', function(e){
			e.preventDefault();
			var answer = confirm('<?php _e( 'Are you sure you want to revoke access to this download?', 'carton' ); ?>');
			if (answer){
				...
				if (product>0) {
					...
					var data = {
						action: 		'carton_revoke_access_to_download',
						product_id: 	product,
						download_id:	file,
						order_id: 		'<?php echo $post->ID; ?>',
 ...
 * @return void
 */
function carton_order_downloads_save( $post_id, $post ) {
	global $wpdb, $carton;
	...
	if ( isset( $_POST['download_id'] ) ) {
		...
		for ( $i = 0; $i < $product_ids_count; $i ++ ) {
            ...
            $data = array(
				...
				'user_email' 			=> carton_clean( $customer_email ),
				'downloads_remaining'	=> carton_clean( $downloads_remaining[$i] )
            );
            ...
            $wpdb->update( $wpdb->prefix . "carton_downloadable_product_permissions",
			    $data,
                array(
					'order_id' 		=> $post_id,
					'product_id' 	=> absint( $product_ids[$i] ),
					'download_id'	=> carton_clean( $download_ids[$i] )
					),
				$format, array( '%d', '%d', '%s' )
			);
...
add_action( 'carton_process_shop_order_meta', 'carton_order_downloads_save', 5, 2 );