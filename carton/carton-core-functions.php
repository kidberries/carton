 ...
 */
add_filter( 'carton_coupon_code', 'sanitize_text_field' );
add_filter( 'carton_coupon_code', 'strtolower' ); // Coupons case-insensitive by default
add_filter( 'carton_stock_amount', 'intval' ); // Stock amounts are integers by default

/**
 * Main function for returning products, uses the CTN_Product_Factory class.
 *
 * @access public
...
/**
 * carton_sanitize_taxonomy_name function.
 *
 * @access public
 * @param mixed $taxonomy
 * @return void
 */
function carton_sanitize_taxonomy_name( $taxonomy ) {
	return str_replace( array( ' ', '_' ), '-', strtolower( $taxonomy ) );
}

/**
 * carton_get_attachment_image_attributes function.
 *
 * @access public
 * @param mixed $attr
 * @return void
 */
function carton_get_attachment_image_attributes( $attr ) {
	if ( strstr( $attr['src'], 'carton_uploads/' ) )
		$attr['src'] = carton_placeholder_img_src();

	return $attr;
}

add_filter( 'wp_get_attachment_image_attributes', 'carton_get_attachment_image_attributes' );


/**
 * carton_prepare_attachment_for_js function.
 *
 * @access public
 * @param mixed $response
 * @return void
 */
function carton_prepare_attachment_for_js( $response ) {

	if ( isset( $response['url'] ) && strstr( $response['url'], 'carton_uploads/' ) ) {
		$response['full']['url'] = carton_placeholder_img_src();
		if ( isset( $response['sizes'] ) ) {
			foreach( $response['sizes'] as $size => $value ) {
				$response['sizes'][ $size ]['url'] = carton_placeholder_img_src();
			}
		}
	}
...
add_filter( 'wp_prepare_attachment_for_js', 'carton_prepare_attachment_for_js' );

/**
 * carton_get_dimension function.
 *
 * Normalise dimensions, unify to cm then convert to wanted unit value
 *
 * Usage: carton_get_dimension(55, 'in');
 *
 * @access public
 * @param mixed $dim
 ...
 * @return float
 */
function carton_get_dimension( $dim, $to_unit ) {
 ...
 * @return string
 */
function carton_placeholder_img_src() {
	global $carton;

	return apply_filters('carton_placeholder_img_src', $carton->plugin_url() . '/assets/images/placeholder.png' );
}
 ...
 * @return string
 */
function carton_placeholder_img( $size = 'shop_thumbnail' ) {
	global $carton;

	$dimensions = $carton->get_image_size( $size );

	return apply_filters('carton_placeholder_img', '<img src="' . carton_placeholder_img_src() . '" alt="Placeholder" width="' . $dimensions['width'] . '" height="' . $dimensions['height'] . '" />' );
}


/**
 * carton_lostpassword_url function.
 *
 * @access public
 * @param mixed $url
 * @return void
 */
function carton_lostpassword_url( $url ) {
    $id = carton_get_page_id( 'lost_password' );
    if ( $id != -1 )
...
add_filter( 'lostpassword_url',  'carton_lostpassword_url' );


/**
 * Send HTML emails from CartoN
 *
 * @access public
 * @param mixed $to
 * @param mixed $subject
...
if ( ! function_exists( 'carton_get_page_id' ) ) {
...
if ( ! function_exists( 'carton_empty_cart' ) ) {
...
if ( ! function_exists( 'carton_disable_admin_bar' ) ) {

	/**
	 * CartoN disable admin bar
	 *
	 * @access public
 ...
 * @return void
 */
function carton_load_persistent_cart( $user_login, $user ) {
	global $carton;

	$saved_cart = get_user_meta( $user->ID, '_carton_persistent_cart', true );

	if ( $saved_cart )
		if ( empty( $carton->session->cart ) || ! is_array( $carton->session->cart ) || sizeof( $carton->session->cart ) == 0 )
			$carton->session->cart = $saved_cart['cart'];
}

/**
 * is_carton - Returns true if on a page which uses CartoN templates (cart and checkout are standard pages with shortcodes and thus are not included)
 *
 * @access public
 * @return bool
 */
...
if ( ! function_exists( 'is_shop' ) ) {
	 ...
	 */
	function is_shop() {
		return ( is_post_type_archive( 'product' ) || is_page( carton_get_page_id( 'shop' ) ) ) ? true : false;
	}
}
 ...
 * @return string
 */
function get_carton_currency_symbol( $currency = '' ) {
	if ( ! $currency )
		$currency = get_carton_currency();
	...
	return apply_filters( 'carton_currency_symbol', $currency_symbol, $currency );
}
...
function get_carton_price_format() {
	$currency_pos = get_option( 'carton_currency_pos' );
	...
	return apply_filters( 'carton_price_format', $format, $currency_pos );
}
 ...
 * @return array
 */
function carton_array_overlay( $a1, $a2 ) {
    foreach( $a1 as $k => $v ) {
        if ( ! array_key_exists( $k, $a2 ) )
        	continue;
        if ( is_array( $v ) && is_array( $a2[ $k ] ) ) {
            $a1[ $k ] = carton_array_overlay( $v, $a2[ $k ] );
        } else {
            $a1[ $k ] = $a2[ $k ];
        }
    }
 ...
 * @return int
 */
function carton_get_term_top_most_parent( $term_id, $taxonomy ) {
 ...
 * @return string
 */
function carton_get_formatted_variation( $variation = '', $flat = false ) {
	global $carton;
	...
	if ( is_array( $variation ) ) {
		...
		foreach ( $variation as $name => $value ) {
			...
			if ( $flat )
				$variation_list[] = $carton->attribute_label(str_replace('attribute_', '', $name)).': '.$value;
			else
				$variation_list[] = '<dt>'.$carton->attribute_label(str_replace('attribute_', '', $name)).':</dt><dd>'.$value.'</dd>';
		}
...
if ( ! function_exists( 'carton_rgb_from_hex' ) ) {
...
if ( ! function_exists( 'carton_hex_darker' ) ) {
...
if ( ! function_exists( 'carton_hex_lighter' ) ) {
...
if ( ! function_exists( 'carton_light_or_dark' ) ) {
	 ...
	 * @return string
	 */
	function carton_light_or_dark( $color, $dark = '#000000', $light = '#FFFFFF' ) {
...
if ( ! function_exists( 'carton_format_hex' ) ) {
 ...
 *
 * The frontend view order pages get around this filter by using remove_filter('comments_clauses', 'carton_exclude_order_comments');
 *
 * @access public
 * @param array $clauses
 * @return array
 */
function carton_exclude_order_comments( $clauses ) {
	global $wpdb, $typenow, $pagenow;

	if ( is_admin() && ( $typenow == 'shop_order' || $pagenow == 'edit-comments.php' ) && current_user_can( 'manage_carton' ) )
...
add_filter( 'comments_clauses', 'carton_exclude_order_comments', 10, 1);
...
add_action( 'comment_feed_join', 'carton_exclude_order_comments_from_feed_join' );
...
add_action( 'comment_feed_where', 'carton_exclude_order_comments_from_feed_where' );
 ...
 * @return void
 */
function carton_downloadable_product_permissions( $order_id ) {
	global $wpdb;

	if (get_post_meta( $order_id, __( 'Download Permissions Granted', 'carton' ), true)==1) return; // Only do this once
	...
	if (sizeof($order->get_items())>0) foreach ($order->get_items() as $item) :
		...
		if ($item['product_id']>0) :
			...
			if ( $_product->exists() && $_product->is_downloadable() ) :
				...
				$file_download_paths = apply_filters( 'carton_file_download_paths', get_post_meta( $product_id, '_file_paths', true ), $product_id, $order_id, $item );
				if ( ! empty( $file_download_paths ) ) {
					foreach ( $file_download_paths as $download_id => $file_path ) {
						carton_downloadable_file_permission( $download_id, $product_id, $order );
					}
				}
	...
	update_post_meta( $order_id,  __( 'Download Permissions Granted', 'carton' ), 1);
}

add_action('carton_order_status_completed', 'carton_downloadable_product_permissions');
 ...
 * @param CTN_Order $order the order
 */
function carton_downloadable_file_permission( $download_id, $product_id, $order ) {
	global $wpdb;
    ...
    $wpdb->insert( $wpdb->prefix . 'carton_downloadable_product_permissions',
        $data,
        $format
    );
}

if ( get_option('carton_downloads_grant_access_after_payment') == 'yes' )
	add_action( 'carton_order_status_processing', 'carton_downloadable_product_permissions' );
...
add_action( 'carton_order_status_completed', 'carton_paying_customer' );
 ...
 * @return string
 */
function carton_product_post_type_link( $permalink, $post ) {
    ...
    if ( empty( $terms ) ) {
        ...
        $product_cat = _x( 'uncategorized', 'slug', 'carton' );
    } else {
...
add_filter( 'post_type_link', 'carton_product_post_type_link', 10, 2 );
...
add_filter( 'terms_clauses', 'carton_terms_clauses', 10, 3);


/**
 * carton_get_product_terms function.
 *
 ...
 * @return array
 */
function carton_get_product_terms( $object_id, $taxonomy, $fields = 'all' ) {
...
/**
 * CartoN Term/Order item Meta API - set table name
 *
 * @access public
 * @return void
 */
...
add_action( 'init', 'carton_taxonomy_metadata_wpdbfix', 0 );
add_action( 'switch_blog', 'carton_taxonomy_metadata_wpdbfix', 0 );


/**
 * CartoN Term Meta API - Update term meta
 *
 * @access public
 * @param mixed $term_id
...
/**
 * CartoN Term Meta API - Add term meta
 *
 * @access public
 * @param mixed $term_id
 * @param mixed $meta_key
...
/**
 * CartoN Term Meta API - Delete term meta
 *
 * @access public
 * @param mixed $term_id
 * @param mixed $meta_key
...
/**
 * CartoN Term Meta API - Get term meta
 *
 * @access public
 * @param mixed $term_id
 * @param mixed $key
...
/**
 * carton_customer_bought_product
 *
...
/**
 * Get capabilities for CartoN - these are assigned to admin/shop manager during installation or reset
 *
 * @access public
 * @return void
 */
...
/**
 * carton_init_roles function.
 *
 * @access public
 * @return void
 */
function carton_init_roles() {
	global $wp_roles;
	...
	if ( is_object( $wp_roles ) ) {
		...
		// Customer role
		add_role( 'shop_customer', __( 'Shop Сustomer', 'carton' ), array(
		    'read' 					=> true,
		    'edit_posts' 				=> false,
		    'delete_posts' 				=> false
		) );
		...
		// Shop manager role
		add_role( 'shop_manager', __( 'Shop Manager', 'carton' ), array(
		    'level_9'                => true,
		    'level_8'                => true,
		    'level_7'                => true,
		    'level_6'                => true,
		...
		add_role( 'shop_administrator', __( 'Shop Administrator', 'carton' ), array(
		    'level_10'               => true,
		    'level_9'                => true,
		    'level_8'                => true,
		    'level_7'                => true,
		...
		$capabilities = carton_get_core_capabilities();
...
/**
 * carton_remove_roles function.
 *
 * @access public
 * @return void
 */
function carton_remove_roles() {
	global $wp_roles;
...
/**
 * carton_delete_order_item function.
 *
 * @access public
 * @param int $item_id
 * @return bool
 */
function carton_delete_order_item( $item_id ) {
	global $wpdb;
	...
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_order_items WHERE order_item_id = %d", $item_id ) );
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_order_itemmeta WHERE order_item_id = %d", $item_id ) );

	do_action( 'carton_delete_order_item', $item_id );

	return true;
}

/**
 * CartoN Order Item Meta API - Update term meta
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $meta_key
...
/**
 * CartoN Order Item Meta API - Add term meta
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $meta_key
...
/**
 * CartoN Order Item Meta API - Delete term meta
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $meta_key
...
/**
 * CartoN Order Item Meta API - Get term meta
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $key
...
/**
 * CartoN Date Format - Allows to change date format for everything CartoN
 *
 * @access public
 * @return string
 */
...
/**
 * CartoN Time Format - Allows to change time format for everything CartoN
 *
 * @access public
 * @return string
 */
 ...
 * @return void
 */
function _carton_term_recount( $terms, $taxonomy, $callback = true, $terms_are_term_taxonomy_ids = true ) {
	global $wpdb;
	...
	// Stock query
	if ( get_option( 'carton_hide_out_of_stock_items' ) == 'yes' ) {
	...
	foreach ( (array) $terms as $term_id => $parent_id ) {
		...
		update_carton_term_meta( $term_id, 'product_count_' . $taxonomy->name, absint( $count ) );
	...
	if ( is_taxonomy_hierarchical( $taxonomy->name ) ) {
		...
		foreach ( (array) $terms as $term ) {
			...
			update_carton_term_meta( $term, 'product_count_' . $taxonomy->name, absint( $count ) );
		}
...
/**
 * carton_recount_after_stock_change function.
 *
 * @access public
 * @return void
 */
function carton_recount_after_stock_change( $product_id ) {
	...
	if ( $product_terms ) {
		...
		_carton_term_recount( $product_tags, get_taxonomy( 'product_tag' ), false, false );

	}

}

add_action( 'carton_product_set_stock_status', 'carton_recount_after_stock_change' );

/**
 * carton_change_term_counts function.
 ...
 * @return void
 */
function carton_change_term_counts( $terms, $taxonomies, $args ) {

	if ( ! in_array( $taxonomies[0], apply_filters( 'carton_change_term_counts', array( 'product_cat', 'product_tag' ) ) ) )
		return $terms;

	$term_counts = $o_term_counts = get_transient( 'ctn_term_counts' );

	foreach ( $terms as &$term ) {
		// If the original term count is zero, there's no way the product count could be higher.
		if ( empty( $term->count ) ) continue;

		$term_counts[ $term->term_id ] = isset( $term_counts[ $term->term_id ] ) ? $term_counts[ $term->term_id ] : get_carton_term_meta( $term->term_id, 'product_count_' . $taxonomies[0] , true );
	...
	if ( $term_counts != $o_term_counts )
		set_transient( 'ctn_term_counts', $term_counts );

	return $terms;
}

if ( ! is_admin() && ! is_ajax() )
	add_filter( 'get_terms', 'carton_change_term_counts', 10, 3 );
...
add_action( 'carton_scheduled_sales', 'carton_scheduled_sales' );


/**
 * carton_cancel_unpaid_orders function.
 *
 * @access public
 * @return void
 */
function carton_cancel_unpaid_orders() {
	global $wpdb;

	$held_duration = get_option( 'carton_hold_stock_minutes' );

	if ( $held_duration < 1 || get_option( 'carton_manage_stock' ) != 'yes' )
		return;
	...
	if ( $unpaid_orders ) {
		foreach ( $unpaid_orders as $unpaid_order ) {
			$order = new CTN_Order( $unpaid_order );

			if ( apply_filters( 'carton_cancel_unpaid_order', true, $order ) )
				$order->update_status( 'cancelled', __( 'Unpaid order cancelled - time limit reached.', 'carton' ) );
		}
	}
...
add_action( 'carton_cancel_unpaid_orders', 'carton_cancel_unpaid_orders' );
