...
/**
 * CartoN Payment Gateway class
 *
 * Extended by individual payment gateways to handle payments.
 *
 * @class 		CTN_Payment_Gateway
 ...
 * @author 		CartonThemes
 */
abstract class CTN_Payment_Gateway extends CTN_Settings_API {
	...
	function get_return_url( $order = '' ) {
		...
		$thanks_page_id = carton_get_page_id('thanks');
		if ( $thanks_page_id ) :
		...
		if ( is_ssl() || get_option('carton_force_ssl_checkout') == 'yes' )
			$return_url = str_replace( 'http:', 'https:', $return_url );

		return apply_filters( 'carton_get_return_url', $return_url );
	}
	 ...
	 */
	function get_title() {
		return apply_filters( 'carton_gateway_title', $this->title, $this->id );
	}
	...
	function get_description() {
		return apply_filters( 'carton_gateway_description', $this->description, $this->id );
	}
	...
	function get_icon() {
		...
		$icon = $this->icon ? '<img src="' . $carton->force_ssl( $this->icon ) . '" alt="' . $this->title . '" />' : '';

		return apply_filters( 'carton_gateway_icon', $icon, $this->id );
	}
	...
	function supports( $feature ) {
		return apply_filters( 'carton_payment_gateway_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
