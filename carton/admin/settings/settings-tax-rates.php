 ...
 * @return void
 */
function carton_tax_rates_setting() {
...
/**
 * carton_tax_rates_setting_save function.
 *
 * @access public
 * @return void
 */
function carton_tax_rates_setting_save() {
	...
	foreach ( $tax_rate_country as $key => $value ) {
		...
		if ( $key == 'new' ) {
			...
			foreach ( $value as $new_key => $new_value ) {
				...
				// Sanitize + format
				$country  = strtoupper( carton_clean( $tax_rate_country[ $key ][ $new_key ] ) );
				$state    = strtoupper( carton_clean( $tax_rate_state[ $key ][ $new_key ] ) );
				$postcode = carton_clean( $tax_rate_postcode[ $key ][ $new_key ] );
				$city     = carton_clean( $tax_rate_city[ $key ][ $new_key ] );
				$rate     = number_format( carton_clean( $tax_rate[ $key ][ $new_key ] ), 4, '.', '' );
				$name     = carton_clean( $tax_rate_name[ $key ][ $new_key ] );
				$priority = absint( carton_clean( $tax_rate_priority[ $key ][ $new_key ] ) );
				...
				if ( ! empty( $postcode ) ) {
					...
					$postcodes = array_map( 'strtoupper', array_map( 'carton_clean', $postcodes ) );
					...
					$wpdb->query( "INSERT INTO {$wpdb->prefix}carton_tax_rate_locations ( location_code, tax_rate_id, location_type ) VALUES " . implode( ',', $postcode_query ) );
				}

				if ( ! empty( $city ) ) {
					$cities = explode( ';', $city );
					$cities = array_map( 'strtoupper', array_map( 'carton_clean', $cities ) );
		...
		// ...whereas the others are updated
			...
			if ( $_POST['remove_tax_rate'][ $key ] == 1 ) {
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_tax_rate_locations WHERE tax_rate_id = %d;", $tax_rate_id ) );
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_tax_rates WHERE tax_rate_id = %d;", $tax_rate_id ) );
				continue;
			}

			// Sanitize + format
			$country  = strtoupper( carton_clean( $tax_rate_country[ $key ] ) );
			$state    = strtoupper( carton_clean( $tax_rate_state[ $key ] ) );
			$rate     = number_format( carton_clean( $tax_rate[ $key ] ), 4, '.', '' );
			$name     = carton_clean( $tax_rate_name[ $key ] );
			$priority = absint( carton_clean( $tax_rate_priority[ $key ] ) );
			...
			if ( isset( $tax_rate_postcode[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'postcode';", $tax_rate_id ) );

				// Add changed
				$postcode  = carton_clean( $tax_rate_postcode[ $key ] );
				$postcodes = explode( ';', $postcode );
				$postcodes = array_map( 'strtoupper', array_map( 'carton_clean', $postcodes ) );
			...
			if ( isset( $tax_rate_city[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}carton_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'city';", $tax_rate_id ) );

				// Add changed
				$city   = carton_clean( $tax_rate_city[ $key ] );
				$cities = explode( ';', $city );
				$cities = array_map( 'strtoupper', array_map( 'carton_clean', $cities ) );
				foreach( $cities as $city ) {
					if ( $city ) {
