 ...
 * Order
 *
 * The CartoN order class handles order data.
 *
 * @class 		CTN_Order
 * @version		1.6.4
...
class CTN_Order {
	...
	public function populate( $result ) {
		...
		$load_data = apply_filters( 'carton_load_order_data', array(
			'order_key'				=> '',
			'billing_first_name'	=> '',
			'billing_last_name' 	=> '',
			'billing_company'		=> '',
	...
	public function get_order_number() {
		return apply_filters( 'carton_order_number', _x( 'OL-', 'hash before order number', 'carton' ) . $this->id, $this );
	}
	...
	public function get_formatted_billing_address() {
		if ( ! $this->formatted_billing_address ) {
			...
			$this->formatted_billing_address = $carton->countries->get_formatted_address( $address );
		}
	...
	public function get_formatted_shipping_address() {
		if ( ! $this->formatted_shipping_address ) {
			if ( $this->shipping_address_1 ) {
				...
				$this->formatted_shipping_address = $carton->countries->get_formatted_address( $address );
			}
		}
	...
	public function get_items( $type = '' ) {
		...
		$line_items = $wpdb->get_results( $wpdb->prepare( "
			...
			FROM 		{$wpdb->prefix}carton_order_items
			WHERE 		order_id = %d
		...
		return apply_filters( 'carton_order_get_items', $items, $this );
	}
	...
	public function get_item_count( $type = '' ) {
		...
		return apply_filters( 'carton_get_item_count', $count, $type, $this );
	}
	...
	public function has_meta( $order_item_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare("SELECT meta_key, meta_value, meta_id, order_item_id
			FROM {$wpdb->prefix}carton_order_itemmeta WHERE order_item_id = %d
	...
	public function get_total_tax() {
		return apply_filters( 'carton_order_amount_total_tax', number_format( (double) $this->order_tax + (double) $this->order_shipping_tax, 2, '.', '' ) );
	}
	...
	public function get_cart_discount() {
		return apply_filters( 'carton_order_amount_cart_discount', number_format( (double) $this->cart_discount, 2, '.', '' ) );
	}
	...
	public function get_order_discount() {
		return apply_filters( 'carton_order_amount_order_discount', number_format( (double) $this->order_discount, 2, '.', '' ) );
	}
	...
	public function get_total_discount() {
		if ( $this->order_discount || $this->cart_discount )
			return apply_filters( 'carton_order_amount_total_discount', number_format( (double) $this->order_discount + (double) $this->cart_discount, 2, '.', '' ) );
	}
	...
	public function get_shipping() {
		return apply_filters( 'carton_order_amount_shipping', number_format( (double) $this->order_shipping, 2, '.', '' ) );
	}
	...
	public function get_shipping_real() {
		return apply_filters( 'carton_order_amount_shipping_real', number_format( (double) $this->order_shipping_real, 2, '.', '' ) );
	}
	...
	public function get_shipping_tax() {
		return apply_filters( 'carton_order_amount_shipping_tax', number_format( (double) $this->order_shipping_tax, 2, '.', '' ) );
	}
	...
	public function get_total() {
		return apply_filters( 'carton_order_amount_total', number_format( (double) $this->order_total, 2, '.', '' ) );
	}
	...
	public function get_shipping_method() {
		return apply_filters( 'carton_order_shipping_method', ucwords( $this->shipping_method_title ) );
	}
	...
	public function get_item_subtotal( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'carton_order_amount_item_subtotal', ($round) ? number_format( $price, 2, '.', '') : $price );
	}
	...
	public function get_line_subtotal( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'carton_order_amount_line_subtotal', ($round) ? number_format( $price, 2, '.', '') : $price );
	}
	...
	public function get_item_total( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'carton_order_amount_item_total', ($round) ? number_format( $price, 2, '.', '') : $price );
	}
	...
	public function get_item_tax( $item, $round = true ) {
		...
		return apply_filters( 'carton_order_amount_item_tax', ($round) ? number_format( $price, 2, '.', '') : $price );
	}
	...
	public function get_line_total( $item, $inc_tax = false ) {
		if ( $inc_tax )
			return apply_filters( 'carton_order_amount_line_total', number_format( $item['line_total'] + $item['line_tax'] , 2, '.', '') );
		else
			return apply_filters( 'carton_order_amount_line_total', number_format( $item['line_total'] , 2, '.', '') );
	}
	...
	public function get_line_tax( $item ) {
		return apply_filters( 'carton_order_amount_line_tax', number_format( $item['line_tax'], 2, '.', '') );
	}
	...
	public function get_formatted_line_subtotal( $item, $tax_display = '' ) {
		...
		if ( $tax_display == 'excl' ) {
			...
			$subtotal = carton_price( $this->get_line_subtotal( $item ), array( 'ex_tax_label' => $ex_tax_label ) );
		} else {
			$subtotal = carton_price( $this->get_line_subtotal( $item, true ) );
		}

		return apply_filters( 'carton_order_formatted_line_subtotal', $subtotal, $item, $this );
	}
	/**
	...
	public function get_formatted_line_total( $item, $tax_display = '' ) {
		...
		if ( $tax_display == 'excl' ) {
			if ( $this->prices_include_tax ) $ex_tax_label = 1; else $ex_tax_label = 0;
			$total = carton_price( $this->get_line_total( $item ), array( 'ex_tax_label' => $ex_tax_label ) );
		} else {
			$total = carton_price( $this->get_line_total( $item, true ) );
		}

		return apply_filters( 'carton_order_formatted_line_total', $total, $item, $this );
	}
	...
	public function get_formatted_line( $item, $tax_display = '' ) {
		...
		return apply_filters( 'carton_order_formatted_line', implode(' ', $line) , $item, $this );
	}
	...
	public function get_formatted_order_total() {
		...
		$formatted_total = carton_price( $this->order_total );

		return apply_filters( 'carton_get_formatted_order_total', $formatted_total, $this );
	}
	...
	public function get_subtotal_to_display( $compound = false, $tax_display = '' ) {
		...
		if ( ! $compound ) {
			...
			$subtotal = carton_price( $subtotal );

			if ( $tax_display == 'excl' && $this->prices_include_tax )
				$subtotal .= ' <small>' . $carton->countries->ex_tax_or_vat() . '</small>';
		...
		return apply_filters( 'carton_order_subtotal_to_display', $subtotal, $compound, $this );
	}
	...
	public function get_shipping_to_display( $tax_display = '' ) {
		...
		if ( $this->order_shipping > 0 ) {
			...
			if ( $tax_display == 'excl' ) {
				...
				$shipping = carton_price( $this->order_shipping );

				if ( $this->order_shipping_tax > 0 && $this->prices_include_tax )
					$tax_text = $carton->countries->ex_tax_or_vat() . ' ';

			} else {

				// Show shipping including tax
				$shipping = carton_price( $this->order_shipping + $this->order_shipping_tax );

				if ( $this->order_shipping_tax > 0 && ! $this->prices_include_tax )
					$tax_text = $carton->countries->inc_tax_or_vat() . ' ';

			}

			$shipping = sprintf( __( '&nbsp;<small>%s: %s</small> ', 'carton' ), $this->get_shipping_method(), $tax_text ) . $shipping;

		} elseif ( $this->get_shipping_method() ) {
			$shipping = $this->get_shipping_method();
		} else {
			$shipping = __( 'Free!', 'carton' );
		}

		return apply_filters( 'carton_order_shipping_to_display', $shipping, $this );
	}
	...
	public function get_cart_discount_to_display() {
		return apply_filters( 'carton_order_cart_discount_to_display', carton_price( $this->get_cart_discount() ), $this );
	}
	...
	public function get_order_discount_to_display() {
		return apply_filters( 'carton_order_discount_to_display', carton_price( $this->get_order_discount() ), $this );
	}
	...
	public function get_order_item_totals( $tax_display = '' ) {
		...
		if ( $subtotal = $this->get_subtotal_to_display() )
			$total_rows['cart_subtotal'] = array(
				'label' => __( 'Cart Subtotal:', 'carton' ),
				'value'	=> $subtotal
			);

		if ( $this->get_cart_discount() > 0 )
			$total_rows['cart_discount'] = array(
				'label' => __( 'Cart Discount:', 'carton' ),
		...
		if ( $this->get_shipping_method() ) {
			if( $this->shipping_method == 'local_pickup' ) {
				$total_rows['shipping'] = array(
					'label' => __( 'Shipping:', 'carton' ) . ' ' . $this->get_shipping_to_display()
				);
			} else {
				$total_rows['shipping'] = array(
					'label' => __( 'Shipping:' ),
			...
			foreach( $fees as $id => $fee ) {
				...
				if ( $tax_display == 'excl' ) {
					...
					$total_rows[ 'fee_' . $id ] = array(
						'label' => $fee['name'],
						'value'	=> carton_price( $fee['line_total'] )
					);
					...
					$total_rows[ 'fee_' . $id ] = array(
						'label' => $fee['name'],
						'value'	=> carton_price( $fee['line_total'] + $fee['line_tax'] )
					);
			...
			if ( sizeof( $this->get_taxes() ) > 0 ) {
				...
				foreach ( $this->get_taxes() as $tax ) {
					...
					$total_rows[ sanitize_title( $tax[ 'name' ] ) ] = array(
						...
						'value'	=> carton_price( ( $tax[ 'tax_amount' ] + $tax[ 'shipping_tax_amount' ] ) )
					);
				}
				...
				$total_rows['tax'] = array(
					...
					'value'	=> carton_price( $this->get_total_tax() )
				);
			}
		}
		...
		if ( $this->get_order_discount() > 0 )
			$total_rows['order_discount'] = array(
				'label' => __( 'Order Discount:', 'carton' ),
        ...
        if( $this->cart_actions_discount ) {
            ...
            foreach( $cart_actions_discounts as $key => $value ) {
                $total_rows[ 'cart_actions_discount_' . sanitize_title( $key ) ] = array(
                    'label' => __( 'Total in Discount Action:', 'carton' ) . ' "' . $key . '"',
                    'value'	=> carton_price( -1 * $value )
                );
            }
        }

		$total_rows['order_total'] = array(
			'label' => __( 'Order Total:', 'carton' ),
		...
		if ( $tax_display == 'incl' ) {
			...
			if ( sizeof( $this->get_taxes() ) > 0 ) {
				...
				foreach ( $this->get_taxes() as $tax ) {
					...
					$tax_string_array[] = sprintf( '%s %s', carton_price( ( $tax[ 'tax_amount' ] + $tax[ 'shipping_tax_amount' ] ) ), isset( $tax[ 'label' ] ) ? $tax[ 'label' ] : $tax[ 'name' ] );
				}

			} elseif ( $this->get_total_tax() > 0 ) {

				$tax_string_array[] = sprintf( '%s %s', carton_price( $this->get_total_tax() ), $carton->countries->tax_or_vat() );

			}

			if ( ! empty( $tax_string_array ) )
				$total_rows['order_total']['value'] .= ' ' . sprintf( __( '(Includes %s)', 'carton' ), implode( ', ', $tax_string_array ) );
		}

		return apply_filters( 'carton_get_order_item_totals', $total_rows, $this );
	}
	...
	public function email_order_items_table( $show_download_links = false, $show_sku = false, $show_purchase_note = false, $show_image = false, $image_size = array( 32, 32 ), $plain_text = false ) {
		...
		carton_get_template( $template, array(
			'order'					=> $this,
		...
		$return = apply_filters( 'carton_email_order_items_table', ob_get_clean() );
	...
	public function is_download_permitted() {
		return apply_filters( 'carton_order_is_download_permitted', $this->status == 'completed' || ( get_option( 'carton_downloads_grant_access_after_payment' ) == 'yes' && $this->status == 'processing' ), $this );
	}
	...
	public function get_checkout_payment_url() {
		...
		$payment_page = get_permalink(carton_get_page_id('pay'));

		if (get_option('carton_force_ssl_checkout')=='yes' || is_ssl()) $payment_page = str_replace('http:', 'https:', $payment_page);

		return apply_filters('carton_get_checkout_payment_url', add_query_arg('pay_for_order', 'true', add_query_arg('order', $this->order_key, add_query_arg('order_id', $this->id, $payment_page))));
	}
	...
	public function get_cancel_order_url() {
		global $carton;
		return apply_filters('carton_get_cancel_order_url', $carton->nonce_url( 'cancel_order', add_query_arg('cancel_order', 'true', add_query_arg('order', $this->order_key, add_query_arg('order_id', $this->id, trailingslashit( home_url() ))))));
	}
	...
	public function get_downloadable_file_urls( $product_id, $variation_id, $item ) {
		...
		return apply_filters( 'carton_get_downloadable_file_urls', $file_urls, $product_id, $variation_id, $item );
	}
	...
	public function add_order_note( $note, $is_customer_note = 0 ) {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) )
			$comment_author_email 	= sanitize_email( strtolower( __( 'CartoN', 'carton' ) ) . '@' . str_replace( 'www.', '', $_SERVER['HTTP_HOST'] ) );
		else
			$comment_author_email 	= sanitize_email( strtolower( __( 'CartoN', 'carton' ) ) . '@noreply.com' );

		$comment_post_ID 		= $this->id;
		$comment_author 		= __( 'CartoN', 'carton' );
		$comment_author_url 	= '';
		$comment_content 		= $note;
		$comment_agent			= 'CartoN';
		...
		if ($is_customer_note) do_action( 'carton_new_customer_note', array( 'order_id' => $this->id, 'customer_note' => $note ) );
	...
	public function update_status( $new_status_slug, $note = '' ) {
		...
		if ( $new_status ) {
			...
			if ( $this->status != $new_status->slug ) {
				...
				// Status was changed
				do_action( 'carton_order_status_' . $new_status->slug, $this->id );
				do_action( 'carton_order_status_' . $this->status . '_to_' . $new_status->slug, $this->id );
				do_action( 'carton_order_status_changed', $this->id, $this->status, $new_status->slug );


				if ( $old_status )
					$this->add_order_note( $note . sprintf( __( 'Order status changed from %s to %s.', 'carton' ), __( $old_status->name, 'carton' ), __( $new_status->name, 'carton' ) ) );
	...
	public function payment_complete() {
		...
		if ( ! empty( $carton->session->order_awaiting_payment ) )
			unset( $carton->session->order_awaiting_payment );
		...
		if ( $this->id && ( $this->status == 'on-hold' || $this->status == 'pending' || $this->status == 'failed' ) ) {
			...
			if ( sizeof( $this->get_items() ) > 0 ) {
				...
				foreach( $this->get_items() as $item ) {
					...
					if ( $item['product_id'] > 0 ) {
						...
						if ( ( $_product->is_downloadable() && $_product->is_virtual() ) || ! apply_filters( 'carton_order_item_needs_processing', true, $_product, $this->id ) ) {
			...
			$new_order_status = apply_filters( 'carton_payment_complete_order_status', $new_order_status, $this->id );
			...
			if ( apply_filters( 'carton_payment_complete_reduce_order_stock', true, $this->id ) )
				$this->reduce_order_stock(); // Payment is complete so reduce stock levels

			do_action( 'carton_payment_complete', $this->id );
		}
	}
	...
	public function reduce_order_stock() {
		...
		if ( get_option('carton_manage_stock') == 'yes' && sizeof( $this->get_items() ) > 0 ) {
			...
			foreach ( $this->get_items() as $item ) {
				...
				if ($item['product_id']>0) {
					...
					if ( $_product && $_product->exists() && $_product->managing_stock() ) {
						...
						$qty = apply_filters( 'carton_order_item_quantity', $item['qty'], $this, $item );

						$new_quantity = $_product->reduce_stock( $qty );

						$this->add_order_note( sprintf( __( 'Item #%s stock reduced from %s to %s.', 'carton' ), $item['product_id'], $old_stock, $new_quantity) );
			...
			do_action( 'carton_reduce_order_stock', $this );

			$this->add_order_note( __( 'Order item stock reduced successfully.', 'carton' ) );
		}
	...
	public function send_stock_notifications( $product, $new_stock, $qty_ordered ) {
		...
		// Backorders
		if ( $new_stock < 0 )
			do_action( 'carton_product_on_backorder', array( 'product' => $product, 'order_id' => $this->id, 'quantity' => $qty_ordered ) );

		// stock status notifications
		$notification_sent = false;

		if ( get_option( 'carton_notify_no_stock' ) == 'yes' && get_option('carton_notify_no_stock_amount') >= $new_stock ) {
			do_action( 'carton_no_stock', $product );
			$notification_sent = true;
		}
		if ( ! $notification_sent && get_option( 'carton_notify_low_stock' ) == 'yes' && get_option('carton_notify_low_stock_amount') >= $new_stock ) {
			do_action( 'carton_low_stock', $product );
			$notification_sent = true;
		}
	...
	public function get_customer_order_notes() {
		...
		remove_filter('comments_clauses', 'carton_exclude_order_comments');
		...
		add_filter('comments_clauses', 'carton_exclude_order_comments');
