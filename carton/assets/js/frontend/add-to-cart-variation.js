...
(function ( $, window, document, undefined ) {
    $(document).ready(function(){
        $('.variations_form').ctn_variation_form();
    });

    $.fn.ctn_variation_form = function () {

        if( ! $(this).attr('data-product_variations') ) {
            $(this)
                .fadeTo('400', '0.4')
                .block({message: null, overlayCSS: {background: 'transparent url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6 } } );

            $.post( carton_params.ajax_url, {action : 'get_data_product_variations', 'product_id':parseInt( $(this).data( 'product_id' ) ) }, function(data) {
                $('.variations_form')
        ...
        $.fn.ctn_variation_form.find_matching_variations = function( product_variations, settings ) {
	        var matching    = [];
            this.variations = {};
			...
			if ( $.fn.ctn_variation_form.variations_match( variation.attributes, settings ) ) {
        ...
        $.fn.ctn_variation_form.variations_match = function( attrs1, attrs2 ) {
	        var match = true;
	...
	// this.find('input[name="variation_id"]').unbind( 'change' );
        ...
        return this
				...
				.on( 'change', 'input[name="variation_id"]', function( event ) {
					if( $(this).val() ) {
						if( typeof $.fn.ctn_variation_form.variations[ $(this).val() ] === 'object' ) {
							var set = $.fn.ctn_variation_form.variations[ $(this).val() ];
				...
				.on( 'change', '.variations select', function( event ) {
					...
					$variation_form
						.trigger( 'carton_variation_select_change' )
				...
				.on( 'focusin', '.variations select', function( event ) {
					...
					$variation_form
						.trigger( 'carton_variation_select_focusin' )
				...
				.on( 'check_variations', function( event, exclude, focus ) {
			        ...
			        var matching_variations = $.fn.ctn_variation_form.find_matching_variations( all_variations, current_settings );
			        ...
			        if ( all_set ) {
			        	...
			        	if ( variation ) {
			            	...
			            	alert( carton_params.i18n_no_matching_variations_text );
				...
				.on( 'update_variation_values', function( event, variations ) {
					...
					$variation_form.trigger('carton_update_variation_values');
