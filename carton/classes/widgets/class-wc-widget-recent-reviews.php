...
class CTN_Widget_Recent_Reviews extends WP_Widget {
	 ...
	 * @return void
	 */
	function CTN_Widget_Recent_Reviews() {

		/* Widget variable settings. */
		$this->woo_widget_cssclass = 'carton widget_recent_reviews';
		$this->woo_widget_description = __( 'Display a list of your most recent reviews on your site.', 'carton' );
		$this->woo_widget_idbase = 'carton_recent_reviews';
		$this->woo_widget_name = __( 'CartoN Recent Reviews', 'carton' );
	 ...
	 */
	 function widget( $args, $instance ) {
		global $comments, $comment, $carton;
		...
		extract($args);
 		...
 		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Reviews', 'carton' ) : $instance['title'], $instance, $this->id_base);
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				...
				printf( _x( 'by %1$s', 'by comment author', 'carton' ), get_comment_author() ) . '</li>';
