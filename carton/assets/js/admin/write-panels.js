jQuery( function($){
	...
	// Type box
	$('.type_box').appendTo( '#carton-product-data h3.hndle span' );

	$(function(){
		// Prevent inputs in meta box headings opening/closing contents
		$('#carton-product-data h3.hndle').unbind('click.postboxes');

		jQuery('#carton-product-data').on('click', 'h3.hndle', function(event){

			// If the user clicks on some form input inside the h3 the box should not be toggled
			if ( $(event.target).filter('input, option, label, select').length )
				return;

			$('#carton-product-data').toggleClass('closed');
		});
	});
	...
	$('#catalog-visibility .save-post-visibility').click(function () {
		...
		if ( $('input[name=_featured]').is(':checked') ) {
			label = label + ', ' + carton_writepanel_params.featured_label
	...
	jQuery("select.ajax_chosen_select_products").ajaxChosen({
	    method: 	'GET',
	    url: 		carton_writepanel_params.ajax_url,
	    dataType: 	'json',
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'carton_json_search_products',
			security: 		carton_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	jQuery("select.ajax_chosen_select_products_and_variations").ajaxChosen({
	    ...
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'carton_json_search_products_and_variations',
			security: 		carton_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	// ORDERS
	jQuery('#carton-order-actions input, #carton-order-actions a').click(function(){
	...
	$('#carton-order-totals').on( 'change', '#_order_tax, #_order_shipping_tax, #_cart_discount, #_order_discount', function() {
		...
		var formatted_total = accounting.formatMoney( total, {
			symbol 		: carton_writepanel_params.currency_format_symbol,
			decimal 	: carton_writepanel_params.currency_format_decimal_sep,
			thousand	: carton_writepanel_params.currency_format_thousand_sep,
			precision 	: carton_writepanel_params.currency_format_num_decimals,
			format		: carton_writepanel_params.currency_format
		} );
	...
	$('button.calc_line_taxes').click(function(){
		// Block write panel
		$('.carton_order_items_wrapper').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(carton_writepanel_params.calc_line_taxes);
		...
		if (answer) {
			...
			var data = {
				action: 		'carton_calc_line_taxes',
				order_id: 		carton_writepanel_params.post_id,
				...
				postcode:		postcode,
				city:			city,
				security: 		carton_writepanel_params.calc_totals_nonce
			};

			$.post( carton_writepanel_params.ajax_url, data, function( response ) {
				...
				$('.carton_order_items_wrapper').unblock();
			});
	...
	$('button.calc_totals').click( function(){
		// Block write panel
		$('#carton-order-totals').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(carton_writepanel_params.calc_totals);
		...
		if (answer) {
			...
			$('#order_items_list tr.item').each(function(){
				...
				if ( carton_writepanel_params.round_at_subtotal=='no' ) {
			...
			// Tax
			if (carton_writepanel_params.round_at_subtotal=='yes') {
		...
		$('#carton-order-totals .calculated').css('background-color', '#e3d2dd');
	}, function() {
		$('#carton-order-totals .calculated').css('background-color', '');
	});

	// Add a line item
	$('#carton-order-items button.add_order_item').click(function(){
		...
		if ( add_item_ids ) {
			...
			$('table.carton_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			$.each( add_item_ids, function( index, value ) {

				var data = {
					action: 		'carton_add_order_item',
					item_to_add: 	value,
					order_id:		carton_writepanel_params.post_id,
					security: 		carton_writepanel_params.order_item_nonce
				};

				$.post( carton_writepanel_params.ajax_url, data, function( response ) {

					$('table.carton_order_items tbody#order_items_list').append( response );
					...
					if (!--count) {
					    ...
					    $('table.carton_order_items').unblock();
					}
	...
	// Add a fee
	$('#carton-order-items button.add_order_fee').click(function(){

		$('table.carton_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var data = {
			action: 		'carton_add_order_fee',
			order_id:		carton_writepanel_params.post_id,
			security: 		carton_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.add_order_item_meta', function(){
		...
		var data = {
			...
			action: 	'carton_add_order_item_meta',
			security: 	carton_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.remove_order_item_meta', function(){
		var answer = confirm( carton_writepanel_params.remove_item_meta )
		if ( answer ) {
			var $row = $(this).closest('tr');

			var data = {
				meta_id: 			$row.attr( 'data-meta_id' ),
				action: 			'carton_remove_order_item_meta',
				security: 			carton_writepanel_params.order_item_nonce
			};
	...
	$('#carton-order-items').on( 'click', 'input.check-column', function() {
		if ( $(this).is(':checked') )
			$('#carton-order-items').find('.check-column input').attr('checked', 'checked');
		else
			$('#carton-order-items').find('.check-column input').removeAttr('checked');
	} );

	$('#carton-order-items').on( 'click', '.do_bulk_action', function() {

		var action = $(this).closest('.bulk_actions').find('select').val();
		var selected_rows = $('#carton-order-items').find('.check-column input:checked');
		var item_ids = [];
		...
		if ( item_ids.length == 0 ) {
			alert( carton_writepanel_params.i18n_select_items );
			return;
		}

		if ( action == 'delete' ) {

			var answer = confirm( carton_writepanel_params.remove_item_notice );
			...
			if ( answer ) {
				...
				var data = {
					...
					action: 			'carton_remove_order_item',
					security: 			carton_writepanel_params.order_item_nonce
				};
			...
			var data = {
				...
				action: 			'carton_reduce_order_item_stock',
				security: 			carton_writepanel_params.order_item_nonce
			};
			...
			var data = {
				...
				order_item_ids: 	item_ids,
				order_item_qty: 	quantities,
				action: 			'carton_increase_order_item_stock',
				security: 			carton_writepanel_params.order_item_nonce
			};
	...
	$('button.load_customer_billing').click(function(){
		...
		var answer = confirm(carton_writepanel_params.load_billing);
		if (answer){
			...
			if (!user_id) {
				alert(carton_writepanel_params.no_customer_selected);
				return false;
			}
			...
			var data = {
				...
				action: 			'carton_get_customer_details',
				security: 			carton_writepanel_params.get_customer_details_nonce
			};

			$(this).closest('.edit_address').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('button.load_customer_shipping').click(function(){
		...
		var answer = confirm(carton_writepanel_params.load_shipping);
		if (answer){
	...
	$('button.billing-same-as-shipping').click(function(){
		var answer = confirm(carton_writepanel_params.copy_billing);
		if (answer){
	...
	$('a.add_tax_row').click(function(){
		...
		var data = {
			order_id: 	carton_writepanel_params.post_id,
			action: 	'carton_add_line_tax',
			security: 	carton_writepanel_params.calc_totals_nonce
		};

		$('#tax_rows').closest('.totals_group').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('select#product-type').change(function(){
		...
		$('body').trigger('carton-product-type-change', select_val, $(this) );
	}).change();
	...
	function show_and_hide_panels() {
		...
		$.each( carton_writepanel_params.product_types, function( index, value ) {
	...
	$('#carton-product-data').on( 'click', '.sale_schedule', function() {
	...
	$('#carton-product-data').on( 'click', '.cancel_sale_schedule', function() {
		var $wrap = $(this).closest( 'div, table' );
	...
	var dates = $( ".sale_price_dates_fields input" ).datepicker({
		...
		showOn: "button",
		buttonImage: carton_writepanel_params.calendar_image,
	...
	// ATTRIBUTE TABLES
		...
		$(".carton_attributes select.multiselect").chosen();

		// Initial order
		var carton_attribute_items = $('.carton_attributes').find('.carton_attribute').get();

		carton_attribute_items.sort(function(a, b) {
		   var compA = parseInt($(a).attr('rel'));
		   var compB = parseInt($(b).attr('rel'));
		   return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
		})
		$(carton_attribute_items).each( function(idx, itm) { $('.carton_attributes').append(itm); } );

		function attribute_row_indexes() {
			$('.carton_attributes .carton_attribute').each(function(index, el){
				$('.attribute_position', el).val( parseInt( $(el).index('.carton_attributes .carton_attribute') ) );
			});
		};
		...
		$('button.add_attribute').on('click', function(){
			...
			var size = $('.carton_attributes .carton_attribute').size();
			...
			if (!attribute_type) {
				...
				$('.carton_attributes').append('<div class="carton_attribute wc-metabox">\
						<h3>\
							<button type="button" class="remove_row button">' + carton_writepanel_params.remove_label + '</button>\
							<div class="handlediv" title="' + carton_writepanel_params.click_to_toggle + '"></div>\
							<strong class="attribute_name"></strong>\
						</h3>\
						<table cellpadding="0" cellspacing="0" class="carton_attribute_data">\
							<tbody>\
								<tr>\
									<td class="attribute_name">\
										<label>' + carton_writepanel_params.name_label + ':</label>\
									...
									<td rowspan="3">\
										<label>' + carton_writepanel_params.values_label + ':</label>\
										<textarea name="attribute_values[' + size + ']" cols="5" rows="5" placeholder="' + carton_writepanel_params.text_attribute_tip + '"></textarea>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<label><input type="checkbox" class="checkbox" ' + ( carton_writepanel_params.default_attribute_visibility ? 'checked="checked"' : '' ) + ' name="attribute_visibility[' + size + ']" value="1" /> ' + carton_writepanel_params.visible_label + '</label>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<div class="enable_variation show_if_variable" ' + enable_variation + '>\
										<label><input type="checkbox" class="checkbox" ' + ( carton_writepanel_params.default_attribute_variation ? 'checked="checked"' : '' ) + ' name="attribute_variation[' + size + ']" value="1" /> ' + carton_writepanel_params.used_for_variations_label + '</label>\
										</div>\
									</td>\
								</tr>\
							</tbody>\
				...
				// Reveal taxonomy row
				var thisrow = $('.carton_attributes .carton_attribute.' + attribute_type);
				$('.carton_attributes').append( $(thisrow) );
				$(thisrow).show().find('.carton_attribute_data').show();
		...
		$('.carton_attributes').on('blur', 'input.attribute_name', function(){
			$(this).closest('.carton_attribute').find('strong.attribute_name').text( $(this).val() );
		});

		$('.carton_attributes').on('click', 'button.select_all_attributes', function(){
			$(this).closest('td').find('select option').attr("selected","selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.carton_attributes').on('click', 'button.select_no_attributes', function(){
			$(this).closest('td').find('select option').removeAttr("selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.carton_attributes').on('click', 'button.remove_row', function() {
			var answer = confirm(carton_writepanel_params.remove_attribute);
			if (answer){
		...
		// Attribute ordering
		$('.carton_attributes').sortable({
			items:'.carton_attribute',
			cursor:'move',
			axis:'y',
			handle: 'h3',
		...
		$('.carton_attributes').on('click', 'button.add_new_attribute', function() {

			$('.carton_attributes').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			var attribute = $(this).attr('data-attribute');
			var $wrapper = $(this).closest('.carton_attribute_data');
			var new_attribute_name = prompt( carton_writepanel_params.new_attribute_prompt );

			if ( new_attribute_name ) {

				var data = {
					action: 		'carton_add_new_attribute',
					taxonomy:		attribute,
					term:			new_attribute_name,
					security: 		carton_writepanel_params.add_attribute_nonce
				};
		...
		$('.save_attributes').on('click', function(){
			...
			var data = {
				post_id: 		carton_writepanel_params.post_id,
				data:			$('.carton_attributes').find('input, select, textarea').serialize(),
				action: 		'carton_save_attributes',
				security: 		carton_writepanel_params.save_attributes_nonce
			};
			...
			$.post( carton_writepanel_params.ajax_url, data, function( response ) {
				...
				this_page = this_page.replace( 'post-new.php?', 'post.php?post=' + carton_writepanel_params.post_id + '&action=edit&' );

				// Load variations panel
				$('#variable_product_options').block({ message: null, overlayCSS: { background: '#fff url(' + carton_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
