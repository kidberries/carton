...
--
-- Name: _transient_ctn_attribute_taxonomies(); Type: FUNCTION; Schema: fn; Owner: wordpress
--

CREATE FUNCTION _transient_ctn_attribute_taxonomies() RETURNS text
    LANGUAGE plperlu
    AS $_$
use JSON;
my $r = spi_exec_query('SELECT * FROM wp_carton_attribute_taxonomies');
return to_json( $r->{rows} );
$_$;


ALTER FUNCTION fn._transient_ctn_attribute_taxonomies() OWNER TO wordpress;
...
--
-- Name: transient_ctn_attribute_taxonomies(); Type: FUNCTION; Schema: fn; Owner: wordpress
--

CREATE FUNCTION transient_ctn_attribute_taxonomies() RETURNS trigger
    LANGUAGE plpgsql
    AS $$BEGIN

  INSERT INTO wp_options (option_name, option_value )
  SELECT '_transient_ctn_attribute_taxonomies'::text, fn.json2php( fn._transient_ctn_attribute_taxonomies() )::text
  WHERE NOT EXISTS (SELECT 1 FROM wp_options WHERE option_name = '_transient_ctn_attribute_taxonomies');

  UPDATE wp_options SET option_value = fn.json2php( fn._transient_ctn_attribute_taxonomies())
  WHERE option_name = '_transient_ctn_attribute_taxonomies';

  RETURN NEW;
END;$$;


ALTER FUNCTION fn.transient_ctn_attribute_taxonomies() OWNER TO wordpress;
