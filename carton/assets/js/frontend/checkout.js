jQuery(document).ready(function($) {
	...
	function update_checkout(custom_data) {
		...
		$('#order_methods, #order_review').block({message: null, overlayCSS: {background: '#fff url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'carton_update_order_review',
			security: 			carton_params.update_order_review_nonce,
	...
	if ( carton_params.option_guest_checkout == 'yes' ) {
	...
	.on( 'blur change', '.input-text, select', function() {
		...
		if ( $parent.is( '.validate-required' ) ) {
			if ( $this.val() == '' ) {
				$parent.removeClass( 'carton-validated' ).addClass( 'carton-invalid carton-invalid-required-field' );
				validated = false;
			}
		}
		...
		if ( $parent.is( '.validate-email' ) ) {
			if ( $this.val() ) {
				...
				if ( ! pattern.test( $this.val()  ) ) {
					$parent.removeClass( 'carton-validated' ).addClass( 'carton-invalid carton-invalid-email' );
					validated = false;
				}
			}
		}
		...
		if ( validated ) {
			$parent.removeClass( 'carton-invalid carton-invalid-required-field' ).addClass( 'carton-validated' );
		}
	} )
	...
	.submit( function() {
		...
		if ( $form.triggerHandler('checkout_place_order') !== false && $form.triggerHandler('checkout_place_order_' + $('#order_review input[name=payment_method]:checked').val() ) !== false ) {
			...
			if ( form_data["blockUI.isBlocked"] != 1 )
				$form.block({message: null, overlayCSS: {background: '#fff url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

			$.ajax({
				type: 		'POST',
				url: 		carton_params.checkout_url,
				data: 		$form.serialize(),
				success: 	function( code ) {
						try {
							// Get the valid JSON only from the returned string
							if ( code.indexOf("<!--CTN_START-->") >= 0 )
								code = code.split("<!--CTN_START-->")[1]; // Strip off before after CTN_START

							if ( code.indexOf("<!--CTN_END-->") >= 0 )
								code = code.split("<!--CTN_END-->")[0]; // Strip off anything after CTN_END
							...
							if (result.result=='success') {
								...
								$('.carton-error, .carton-message').remove();
	...
	$('form.checkout_coupon').submit( function() {
		...
		$form.addClass('processing').block({message: null, overlayCSS: {background: '#fff url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'carton_apply_coupon',
			security: 			carton_params.apply_coupon_nonce,
	...
	/* Localisation */
	var locale_json = carton_params.locale.replace(/&quot;/g, '"');
	var locale = $.parseJSON( locale_json );
	var required = ' <abbr class="required" title="' + carton_params.i18n_required_text + '">*</abbr>';
	...
	.bind('update_checkout_fields', function() {
        ...
        $('#checkout_billing_address')
            .addClass('processing')
            .block({message: null, overlayCSS: {background: '#fff url(' + carton_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}})
	...
	// Update on page load
	if ( carton_params.is_checkout == 1 ) {
