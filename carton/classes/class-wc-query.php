<?php
/**
 * Contains the query functions for CartoN which alter the front-end post queries and loops.
 *
 * @class 		CTN_Query
 * @version		1.6.4
...
class CTN_Query {
	...
	public function pre_get_posts( $q ) {
		...
		if ( $q->is_home() && 'page' == get_option('show_on_front') && get_option('page_on_front') == carton_get_page_id('shop') ) {
		...
		if ( $q->is_page() && 'page' == get_option( 'show_on_front' ) && $q->get('page_id') == carton_get_page_id('shop') ) {
		...
		// We're on a shop page so queue the carton_get_products_in_view function
	...
	public function search_post_excerpt( $where = '' ) {
		...
		if ( empty( $wp_the_query->query_vars['ctn_query'] ) )
		    return $where;
	...
	public function wpseo_metadesc() {
		return wpseo_get_value( 'metadesc', carton_get_page_id('shop') );
	}
	...
	public function wpseo_metakey() {
		return wpseo_get_value( 'metakey', carton_get_page_id('shop') );
	}
	...
	public function product_query( $q ) {
		...
		$q->set( 'ctn_query', true );

		// Store variables
		$this->post__in = $post__in;
		$this->meta_query = $meta_query;

		do_action( 'carton_product_query', $q, $this );
	}
	...
	public function get_products_in_view() {
		...
		$transient_name = 'ctn_uf_pid_' . md5( http_build_query( $current_wp_query ) );
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			$orderby_value = isset( $_GET['orderby'] ) ? carton_clean( $_GET['orderby'] ) : apply_filters( 'carton_default_catalog_orderby', get_option( 'carton_default_catalog_orderby' ) );
		...
		return apply_filters( 'carton_get_catalog_ordering_args', $args );
	}
