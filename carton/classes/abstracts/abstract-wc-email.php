 ...
 *
 * CartoN Email Class which is extended by specific email template classes to add emails to CartoN
 *
 * @class 		CTN_Email
 * @version		2.0.0
 ...
 */
abstract class CTN_Email extends CTN_Settings_API {
	...
	function __construct() {
		...
		// Save settings hook
		add_action( 'carton_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );

		// Default template base if not declared in child constructor
		if ( is_null( $this->template_base ) )
			$this->template_base = $carton->plugin_path() . '/templates/';
	...
	function get_subject() {
		return apply_filters( 'carton_email_subject_' . $this->id, $this->format_string( $this->subject ), $this->object );
	}
	...
	function get_heading() {
		return apply_filters( 'carton_email_heading_' . $this->id, $this->format_string( $this->heading ), $this->object );
	}
	...
	function get_recipient() {
		return apply_filters( 'carton_email_recipient_' . $this->id, $this->recipient, $this->object );
	}
	...
	function get_headers() {
		return apply_filters( 'carton_email_headers', "Content-Type: " . $this->get_content_type() . "\r\n", $this->id, $this->object );
	}
	...
	function get_attachments() {
		return apply_filters( 'carton_email_attachments', '', $this->id, $this->object );
	}
	...
	function is_enabled() {
		...
		return apply_filters( 'carton_email_enabled_' . $this->id, $enabled, $this->object );
	}
	...
	function style_inline( $content ) {
		...
		foreach( $nodes_h1 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'carton_email_text_color' ) . "; display:block; font-family:Arial; font-size:34px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		foreach( $nodes_h2 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'carton_email_text_color' ) . "; display:block; font-family:Arial; font-size:30px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		foreach( $nodes_h3 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'carton_email_text_color' ) . "; display:block; font-family:Arial; font-size:26px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		$nodes = $dom->getElementsByTagName('a');

		foreach( $nodes as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'carton_email_text_color' ) . "; font-weight:normal; text-decoration:underline;" );
	...
	function get_from_name() {
		return wp_specialchars_decode( esc_html( get_option( 'carton_email_from_name' ) ) );
	}
	...
	function get_from_address() {
		return sanitize_email( get_option( 'carton_email_from_address' ) );
	}
    ...
    public function process_admin_options() {
		...
		if ( ! empty( $_POST['template_html_code'] ) && ! empty( $this->template_html ) ) {
			...
			$saved	= false;
			$file	= get_stylesheet_directory() . '/carton/' . $this->template_html;
			...
			if ( ! $saved ) {
    			$redirect = add_query_arg( 'ctn_error', urlencode( __( 'Could not write to template file.', 'carton' ) ) );
		...
		if ( ! empty( $_POST['template_plain_code'] ) && ! empty( $this->template_plain ) ) {
    		...
    		$saved	= false;
			$file	= get_stylesheet_directory() . '/carton/' . $this->template_plain;
			...
			if ( ! $saved ) {
    			$redirect = add_query_arg( 'ctn_error', __( 'Could not write to template file.', 'carton' ) );
    			wp_redirect( $redirect );
    			exit;
    		}
		}
	...
	function admin_options() {
		...
		if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) {
			...
			if ( ! empty( $_GET['move_template'] ) && ( $template = esc_attr( basename( $_GET['move_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if (  wp_mkdir_p( dirname( get_stylesheet_directory() . '/carton/' . $this->$template ) ) && ! file_exists( get_stylesheet_directory() . '/carton/' . $this->$template ) ) {
						copy( $this->template_base . $this->$template, get_stylesheet_directory() . '/carton/' . $this->$template );
						echo '<div class="updated fade"><p>' . __( 'Template file copied to theme.', 'carton' ) . '</p></div>';
					}
				}
			}
			...
			if ( ! empty( $_GET['delete_template'] ) && ( $template = esc_attr( basename( $_GET['delete_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if ( file_exists( get_stylesheet_directory() . '/carton/' . $this->$template ) ) {
						unlink( get_stylesheet_directory() . '/carton/' . $this->$template );
						echo '<div class="updated fade"><p>' . __( 'Template file deleted from theme.', 'carton' ) . '</p></div>';
					}
				}
			}
		...
		?>
		<h3><?php echo ( ! empty( $this->title ) ) ? $this->title : __( 'Settings','carton' ) ; ?></h3>
		...
		<?php if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) { ?>
			...
			<?php
				$templates = array(
					'template_html' 	=> __( 'HTML template', 'carton' ),
					'template_plain' 	=> __( 'Plain text template', 'carton' )
				);
				foreach ( $templates as $template => $title ) :
					if ( empty( $this->$template ) )
						continue;

					$local_file = get_stylesheet_directory() . '/carton/' . $this->$template;
					...
					<div class="template <?php echo $template; ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							...
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : ?>
									<a href="<?php echo remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template ) ); ?>" class="delete_template button"><?php _e( 'Delete template file', 'carton' ); ?></a>
								<?php endif; ?>

								<?php printf( __( 'This template has been overridden by your theme and can be found in: <code>%s</code>.', 'carton' ), 'yourtheme/carton/' . $this->$template ); ?>
							</p>
						...
						<?php elseif ( file_exists( $core_file ) ) : ?>
							...
							<p>
								...
								<?php if ( ( is_dir( get_stylesheet_directory() . '/carton/emails/' ) && is_writable( get_stylesheet_directory() . '/carton/emails/' ) ) || is_writable( get_stylesheet_directory() ) ) : ?>
									<a href="<?php echo remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template ) ); ?>" class="button"><?php _e( 'Copy file to theme', 'carton' ); ?></a>
								<?php endif; ?>

								<?php printf( __( 'To override and edit this email template copy <code>%s</code> to your theme folder: <code>%s</code>.', 'carton' ), plugin_basename( $core_file ) , 'yourtheme/carton/' . $this->$template ); ?>
							</p>
						...
						<?php else : ?>
							...
							<p><?php _e( 'File was not found.', 'carton' ); ?></p>
			...
			$carton->add_inline_js("
				...
				var view = '" . __( 'View template', 'carton' ) . "';
				var hide = '" . __( 'Hide template', 'carton' ) . "';
				...
				jQuery('a.delete_template').click(function(){
					var answer = confirm('" . __( 'Are you sure you want to delete this template file?', 'carton' ) . "');
