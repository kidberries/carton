...
/**
 * CartoN Shipping Method Class
 *
 * Extended by shipping methods to handle shipping calculations etc.
 *
 * @class 		CTN_Shipping_Method
 * @version		1.6.4
 * @package		CartoN/Abstracts
 * @category	Abstract Class
 * @author 		CartonThemes
 */
abstract class CTN_Shipping_Method extends CTN_Settings_API {
	...
	/** @var bool Whether the method has settings or not (In CartoN > Settings > Shipping) */
	...
	function add_rate( $args = array() ) {
		...
		if ( ! is_array( $taxes ) && $taxes !== false && $total_cost > 0 && get_option( 'carton_calc_taxes' ) == 'yes' && $this->tax_status == 'taxable' ) {
		...
		$this->rates[] = new CTN_Shipping_Rate( $id, $label, $total_cost, $total_cost_real, $taxes, $this->id, $label_extra, $info );
	}
    ...
    function is_available( $package ) {
		...
		if ( isset( $carton->cart->cart_contents_total ) && isset( $this->min_amount ) && $this->min_amount && $this->min_amount > $carton->cart->cart_contents_total )
			return false;
		...
		return apply_filters( 'carton_shipping_' . $this->id . '_is_available', true, $package );
    }
	 ...
	 */
	function get_title() {
		return apply_filters( 'carton_shipping_method_title', $this->title, $this->id );
	}
...
//echo "<pre>";
	...
	function supports( $feature ) {
		return apply_filters( 'carton_shipping_method_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
}
