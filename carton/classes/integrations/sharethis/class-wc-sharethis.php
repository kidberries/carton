 ...
 * @author 		CartonThemes
 */
class CTN_ShareThis extends CTN_Integration {
	...
	public function __construct() {
        $this->id					= 'sharethis';
        $this->method_title     	= __( 'ShareThis', 'carton' );
        $this->method_description	= __( 'ShareThis offers a sharing widget which will allow customers to share links to products with their friends.', 'carton' );
	...
	<span class="st_twitter"></span><span class="st_email"></span><span class="st_sharethis" st_image="{image}"></span><span class="st_plusone_button"></span>
		...
		// Actions
		add_action( 'carton_update_options_integration_sharethis', array( $this, 'process_admin_options' ) );

		// Share widget
		add_action( 'carton_share', array( $this, 'sharethis_code' ) );
    }
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			'publisher_id' => array(
				'title' 		=> __( 'ShareThis Publisher ID', 'carton' ),
				'description' 	=> sprintf( __( 'Enter your %1$sShareThis publisher ID%2$s to show social sharing buttons on product pages.', 'carton' ), '<a href="http://sharethis.com/account/">', '</a>' ),
				'type' 			=> 'text',
				'default' 		=> get_option('carton_sharethis')
			),
			'sharethis_code' => array(
				'title' 		=> __( 'ShareThis Code', 'carton' ),
				'description' 	=> __( 'You can tweak the ShareThis code by editing this option.', 'carton' ),
				'type' 			=> 'textarea',
...
function add_sharethis_integration( $integrations ) {
	$integrations[] = 'CTN_ShareThis';
	return $integrations;
}

add_filter('carton_integrations', 'add_sharethis_integration' );