 ...
 * @return array
 */
function carton_edit_discount_columns($columns){

	$columns = array();

	$columns["cb"] 			= "<input type=\"checkbox\" />";
	$columns["title"] = __( 'Discount Name', 'carton' );
	//$columns["type"] 		= __( 'Discount type', 'carton' );
	//$columns["amount"] 		= __( 'Discount amount', 'carton' );
	$columns["description"] = __( 'Discount Description', 'carton' );
	//$columns["products"]	= __( 'Product IDs', 'carton' );
	//$columns["usage"] 		= __( 'Usage / Limit', 'carton' );
	//$columns["expiry_date"] = __( 'Expiry date', 'carton' );
        $columns["time_inf"] = __( 'Активность в течение суток', 'carton' );
        $columns["date_inf"] = __( 'Активность на данную дату', 'carton' );
	return $columns;
}

add_filter( 'manage_edit-shop_discount_columns', 'carton_edit_discount_columns' );
 ...
 * @return void
 */
function carton_custom_discount_columns( $column, $level = 0 ) {
	global $post, $carton;
	...
	switch ( $column ) {
		case "title" :
			...
			echo '<div class="code tips" data-tip="' . __( 'Edit discount', 'carton' ) . '"><a href="' . esc_attr( $edit_link ) . '"><span>' . esc_html( $title ). '</span></a></div>';
		...
		break;
		case "type" :
			echo esc_html( $carton->get_discount_discount_type( get_post_meta( $post->ID, 'discount_type', true ) ) );
		break;
		case "times" :
...
add_action( 'manage_shop_discount_posts_custom_column', 'carton_custom_discount_columns', 2 );
 ...
 * @return void
 */
function carton_restrict_manage_discounts() {
	...
	<select name='discount_type' id='dropdown_shop_discount_type'>
		...
		<?php
			$types = $carton->get_discount_discount_types();
...
add_action( 'restrict_manage_posts', 'carton_restrict_manage_discounts' );
 ...
 * @return array
 */
function carton_discounts_by_type_query( $vars ) {
	global $typenow, $wp_query;
    if ( $typenow == 'shop_discount' && ! empty( $_GET['discount_type'] ) ) {

		$vars['meta_key'] = 'discount_type';
		$vars['meta_value'] = carton_clean( $_GET['discount_type'] );
...
add_filter( 'request', 'carton_discounts_by_type_query' );