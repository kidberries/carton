 ...
 * Implements the CTN_Session abstract class
 *
 ...
 *
 * @class 		CTN_Session_Handler
 * @version		2.0.0
 * @package		CartoN/Classes
 * @category	Class
 * @author 		CartonThemes
 */
class CTN_Session_Handler extends CTN_Session {
	...
	public function __construct() {
		$this->_cookie = 'ctn_session_cookie_' . COOKIEHASH;
		...
		if ( $cookie = $this->get_session_cookie() ) {
			...
			if ( time() > $this->_session_expiring ) {
				...
				update_option( '_ctn_session_expires_' . $this->_customer_id, $this->_session_expiration );
			}
    	...
    	// Actions
    	add_action( 'carton_cleanup_sessions', array( $this, 'cleanup_sessions' ), 10 );
    ...
    private function set_session_expiration() {
	    $this->_session_expiring    = time() + intval( apply_filters( 'ctn_session_expiring', 60 * 60 * 47 ) ); // 47 Hours
		$this->_session_expiration  = time() + intval( apply_filters( 'ctn_session_expiration', 60 * 60 * 48 ) ); // 48 Hours
    }
	...
	private function get_session_data() {
		return get_option( '_ctn_session_' . $this->_customer_id, array() );
	}
    ...
    public function save_data() {
    	...
    	if ( $this->_dirty ) {
    		...
    		$session_option = '_ctn_session_' . $this->_customer_id;
    		$session_expiry_option = '_ctn_session_expires_' . $this->_customer_id;
	...
	public function cleanup_sessions() {
		...
		$session_names = $wpdb->get_col( $wpdb->prepare( "
			...
			FROM
				{$wpdb->options} a
			WHERE
				a.option_name LIKE '_ctn_session_expires_%%'
		...
		$wpdb->query( $wpdb->prepare( "
			DELETE
			FROM {$wpdb->options}
			WHERE regexp_replace(option_name, '_ctn_session.*_([^_]+)', '\1', '') IN (
			  SELECT regexp_replace( option_name, '_ctn_session.*_([^_]+)', '\1', '')
			  FROM {$wpdb->options}
			  WHERE
			    option_name ~ '^_ctn_session_' AND
			    option_value < %s
			);
		", $now ) );
	}
