 ...
 * @return void
 */
function carton_order_data_meta_box($post) {
	global $post, $wpdb, $thepostid, $theorder, $order_status, $carton;
	...
	if ( $order_status ) {
		...
		$order_status = sanitize_title( apply_filters( 'carton_default_order_status', 'pending' ) );
	}
	...
	<div class="panel-wrap carton">
		...
		<div id="order_data" class="panel">
			...
			<p class="order_number"><?php
				...
				echo __( 'Order number', 'carton' ) . ' ' . esc_html( $order->get_order_number() ) . '. ';

				$ip_address = get_post_meta( $post->ID, '_customer_ip_address', true );

				if ( $ip_address )
					echo __( 'Customer IP:', 'carton' ) . ' ' . esc_html( $ip_address );
			...
			<div class="order_data_column_container">
				<div class="order_data_column">
					...
					<h4><?php _e( 'General Details', 'carton' ); ?></h4>

					<p class="form-field"><label for="order_status"><?php _e( 'Order status:', 'carton' ) ?></label>
					<select id="order_status" name="order_status" class="chosen_select">
						<?php
							$statuses = (array) get_terms( 'shop_order_status', array( 'hide_empty' => 0, 'orderby' => 'id' ) );
							foreach ( $statuses as $status ) {
								echo '<option value="' . esc_attr( $status->slug ) . '" ' . selected( $status->slug, $order_status, false ) . '>' . esc_html__( $status->name, 'carton' ) . '</option>';
							}
						?>
					</select></p>

					<p class="form-field last"><label for="order_date"><?php _e( 'Order Date:', 'carton' ) ?></label>
						<input type="text" class="date-picker-field" name="order_date" id="order_date" maxlength="10" value="<?php echo date_i18n( 'Y-m-d', strtotime( $post->post_date ) ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" /> @ <input type="text" class="hour" placeholder="<?php _e( 'h', 'carton' ) ?>" name="order_date_hour" id="order_date_hour" maxlength="2" size="2" value="<?php echo date_i18n( 'H', strtotime( $post->post_date ) ); ?>" pattern="\-?\d+(\.\d{0,})?" />:<input type="text" class="minute" placeholder="<?php _e( 'm', 'carton' ) ?>" name="order_date_minute" id="order_date_minute" maxlength="2" size="2" value="<?php echo date_i18n( 'i', strtotime( $post->post_date ) ); ?>" pattern="\-?\d+(\.\d{0,})?" />
					</p>

					<p class="form-field form-field-wide">
						<label for="customer_user"><?php _e( 'Customer:', 'carton' ) ?></label>
						<select id="customer_user" name="customer_user" class="ajax_chosen_select_customer">
							<option value=""><?php _e( 'Guest', 'carton' ) ?></option>
							<?php
								if ( $customer_user ) {
						...
						$carton->add_inline_js( "
							jQuery('select.ajax_chosen_select_customer').ajaxChosen({
							    ...
							    minTermLength: 	1,
							    data:		{
							    	action: 	'carton_json_search_customers',
					...
					<?php if ( get_option( 'carton_enable_order_comments' ) != 'no' ) : ?>
						...
						<p class="form-field form-field-wide"><label for="excerpt"><?php _e( 'Customer Note:', 'carton' ) ?></label>
						<textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt" placeholder="<?php _e( 'Customer\'s notes about the order', 'carton' ); ?>"><?php echo wp_kses_post( $post->post_excerpt ); ?></textarea></p>

					<?php endif; ?>

					<?php do_action( 'carton_admin_order_data_after_order_details', $order ); ?>

				</div>
				<div class="order_data_column">
					<h4><?php _e( 'Billing Details', 'carton' ); ?> <a class="edit_address" href="#">(<?php _e( 'Edit', 'carton' ) ;?>)</a></h4>
					<?php
						$billing_data = apply_filters('carton_admin_billing_fields', array(
							'first_name' => array(
							...
							'country' => array(
								...
								'show'	=> false,
								'type'	=> 'select',
								'options' => array( '' => __( 'Select a country&hellip;', 'carton' ) ) + $carton->countries->get_allowed_countries()
								),
							'state' => array(
						...
						echo '<div class="address">';
							...
							if ( $order->get_formatted_billing_address() )
								echo '<p><strong>' . __( 'Address', 'carton' ) . ':</strong><br/> ' . $order->get_formatted_billing_address() . '</p>';
							else
								echo '<p class="none_set"><strong>' . __( 'Address', 'carton' ) . ':</strong> ' . __( 'No billing address set.', 'carton' ) . '</p>';
						...
						// Display form
						echo '<div class="edit_address"><p><button class="button load_customer_billing">'.__( 'Load billing address', 'carton' ).'</button></p>';
						...
						foreach ( $billing_data as $key => $field ) {
							...
							switch ( $field['type'] ) {
								case "select" :
									carton_wp_select( array( 'id' => '_billing_' . $key, 'label' => $field['label'], 'options' => $field['options'] ) );
								break;
								default :
									carton_wp_text_input( array( 'id' => '_billing_' . $key, 'label' => $field['label'] ) );
								break;
							}
						}
						...
						do_action( 'carton_admin_order_data_after_billing_address', $order );
					?>
				</div>
				<div class="order_data_column">

					<h4><?php _e( 'Shipping Details', 'carton' ); ?> <a class="edit_address" href="#">(<?php _e( 'Edit', 'carton' ) ;?>)</a></h4>
					<?php
						$shipping_data = apply_filters('carton_admin_shipping_fields', array(
							'first_name' => array(
								'label' => __( 'First Name', 'carton' ),
								'show'	=> false
								),
						...
						echo '<div class="address">';
							...
							if ( $order->get_formatted_shipping_address() )
								echo '<p><strong>' . __( 'Address', 'carton' ) . ':</strong><br/> ' . $order->get_formatted_shipping_address() . '</p>';
							else
								echo '<p class="none_set"><strong>' . __( 'Address', 'carton' ) . ':</strong> ' . __( 'No shipping address set.', 'carton' ) . '</p>';
						...
						// Display form
						echo '<div class="edit_address"><p><button class="button load_customer_shipping">' . __( 'Load shipping address', 'carton' ) . '</button> <button class="button billing-same-as-shipping">'. __( 'Copy from billing', 'carton' ) . '</button></p>';
						...
						if ( $shipping_data ) foreach ( $shipping_data as $key => $field ) {
							...
							switch ( $field['type'] ) {
								case "select" :
									carton_wp_select( array( 'id' => '_shipping_' . $key, 'label' => $field['label'], 'options' => $field['options'] ) );
								break;
								default :
									carton_wp_text_input( array( 'id' => '_shipping_' . $key, 'label' => $field['label'] ) );
								break;
							}
						}
						...
						do_action( 'carton_admin_order_data_after_shipping_address', $order );
					?>
				</div>
			</div>
			<div class="clear"></div>
 ...
 * @return void
 */
function carton_order_actions_meta_box( $post ) {
	global $carton, $theorder, $wpdb;

	if ( ! is_object( $theorder ) )
		$theorder = new CTN_Order( $post->ID );

	$order = $theorder;
	?>
	<ul class="order_actions submitbox">

		<?php do_action( 'carton_order_actions_start', $post->ID ); ?>

		<li class="wide" id="actions">
			<select name="ctn_order_action">
				<option value=""><?php _e( 'Actions', 'carton' ); ?></option>
				<optgroup label="<?php _e( 'Resend order emails', 'carton' ); ?>">
					<?php
					global $carton;
					$mailer = $carton->mailer();

					$available_emails = apply_filters( 'carton_resend_order_emails_available', array( 'new_order', 'customer_processing_order', 'customer_completed_order', 'customer_invoice' ) );
				...
				<?php foreach( apply_filters( 'carton_order_actions', array() ) as $action => $title ) { ?>
					<option value="<?php echo $action; ?>"><?php echo $title; ?></option>
				<?php } ?>
			</select>

			<button class="button wc-reload" title="<?php _e( 'Apply', 'carton' ); ?>"><span><?php _e( 'Apply', 'carton' ); ?></span></button>
		</li>
		...
		<li class="wide">
			<div id="delete-action"><?php
				if ( current_user_can( "delete_post", $post->ID ) ) {
					if ( ! EMPTY_TRASH_DAYS )
						$delete_text = __( 'Delete Permanently', 'carton' );
					else
						$delete_text = __( 'Move to Trash', 'carton' );
					?><a class="submitdelete deletion" href="<?php echo esc_url( get_delete_post_link( $post->ID ) ); ?>"><?php echo $delete_text; ?></a><?php
				}
			?></div>

			<input type="submit" class="button save_order button-primary tips" name="save" value="<?php _e( 'Save Order', 'carton' ); ?>" data-tip="<?php _e( 'Save/update the order', 'carton' ); ?>" />
		</li>

		<?php do_action( 'carton_order_actions_end', $post->ID ); ?>
 ...
 * @return void
 */
function carton_order_totals_meta_box( $post ) {
	global $carton, $theorder, $wpdb;
	...
	<div class="totals_group">
		<h4><span class="discount_total_display inline_total"></span><?php _e( 'Discounts', 'carton' ); ?></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Cart Discount:', 'carton' ); ?>&nbsp;<a class="tips" data-tip="<?php _e( 'Discounts before tax - calculated by comparing subtotals to totals.', 'carton' ); ?>" href="#">[?]</a></label>
			...
			<li class="right">
				<label><?php _e( 'Order Discount:', 'carton' ); ?>&nbsp;<a class="tips" data-tip="<?php _e( 'Discounts after tax - user defined.', 'carton' ); ?>" href="#">[?]</a></label>
		...
		<?php
			...
			foreach ( $coupons as $item_id => $item ) {
				...
				echo '<li class="tips code" data-tip="' . esc_attr( carton_price( $item['discount_amount'] ) ) . '"><a href="' . $link . '"><span>' . esc_html( $item['name'] ). '</span></a></li>';
	...
	<div class="totals_group">
		<h4><?php _e( 'Shipping', 'carton' ); ?></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Label:', 'carton' ); ?></label>
				<input type="text" id="_shipping_method_title" name="_shipping_method_title" placeholder="<?php _e( 'The shipping title the customer sees', 'carton' ); ?>" value="<?php
			...
			<li class="right">
				<label><?php _e( 'Method:', 'carton' ); ?></label>
				<select name="_shipping_method" id="_shipping_method" class="first">
					<option value=""><?php _e( 'N/A', 'carton' ); ?></option>
					<?php
						...
						if ( ! $found_method && ! empty( $chosen_method ) ) {
							echo '<option value="' . esc_attr( $chosen_method ) . '" selected="selected">' . __( 'Other', 'carton' ) . '</option>';
						} else {
							echo '<option value="other">' . __( 'Other', 'carton' ) . '</option>';
						}
					?>
				</select>
			</li>
			...
			<li class="wide">
				<?php
					...
					carton_wp_textarea_input( array(
						'id'          => '_order_shipping_customer_instruction',
						'class'       => 'large disabled',
						'label'       => __( 'Instruction (for Customer):', 'carton' ),
						'description' => __( 'Pickup Instruction', 'carton' ),
						'value'       => $instruction,
			...
			<li class="left">
				<label><?php _e( 'Cost (for Customer):', 'carton' ); ?></label>
				<input type="number" step="any" min="0" id="_order_shipping" name="_order_shipping" placeholder="0.00 <?php _e( '(ex. tax)', 'carton' ); ?>" value="<?php
			...
			<li class="right">
				<label><?php _e( 'Cost (Real):', 'carton' ); ?></label>
				<input type="number" step="any" min="0" id="_order_shipping_real" name="_order_shipping_real" placeholder="0.00 <?php _e( '(ex. tax)', 'carton' ); ?>" value="<?php
			...
			<li class="left">
				<label><?php _e( 'Tracking Number:', 'carton' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Label for Tracking Number:', 'carton' ); ?></label>
			...
			<li class="left">
				<label><?php _e( 'Shipping Status:', 'carton' ); ?></label>
				<input type="text" id="_order_shipping_tracking_status" name="_order_shipping_tracking_status" placeholder="<?php _e( 'Created', 'carton' ); ?>" value="<?php
		...
		<?php do_action( 'carton_admin_order_totals_after_shipping', $post->ID ) ?>
		<div class="clear"></div>
	</div>
	...
	<div class="totals_group tax_rows_group">
		...
		<h4><a href="#" class="add_tax_row"><?php _e( '+ Add tax row', 'carton' ); ?> <span class="tips" data-tip="<?php _e( 'These rows contain taxes for this order. This allows you to display multiple or compound taxes rather than a single total.', 'carton' ); ?>">[?]</span></a></a></h4>
		<div class="clear"></div>
	</div>
	<div class="totals_group">
		<h4><span class="tax_total_display inline_total"></span><?php _e( 'Tax Totals', 'carton' ); ?></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Sales Tax:', 'carton' ); ?>&nbsp;<a class="tips" data-tip="<?php _e( 'Total tax for line items + fees.', 'carton' ); ?>" href="#">[?]</a></label>
			...
			<li class="right">
				<label><?php _e( 'Shipping Tax:', 'carton' ); ?></label>
	...
	<div class="totals_group">
		<h4><?php _e( 'Order Totals', 'carton' ); ?></h4>
		<ul class="totals">

			<li class="left">
				<label><?php _e( 'Order Total:', 'carton' ); ?></label>
			...
			<li class="right">
				<label><?php _e( 'Payment Method:', 'carton' ); ?></label>
	...
	<p class="buttons">
		<?php if ( get_option( 'carton_calc_taxes' ) == 'yes' ) : ?>
			<button type="button" class="button calc_line_taxes"><?php _e( 'Calc taxes', 'carton' ); ?></button>
		<?php endif; ?>
		<button type="button" class="button calc_totals button-primary"><?php _e( 'Calc totals', 'carton' ); ?></button>
	</p>
	<?php
}
...
add_action( 'carton_process_shop_order_meta', 'carton_process_shop_order_meta', 10, 2 );
